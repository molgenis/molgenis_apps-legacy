...
public class ImportCbmData_omx extends PluginModel<Entity>
{
	...
	public ImportCbmData_omx(String name, ScreenController<?> parent)
	{
		super(name, parent);
	}
	...
	{
		return "plugin_ImportCbmData_omx";
	}

	@Override
	public String getViewTemplate()
	{
		return "plugin/ImportCbmData_omx.ftl";
	}
	...
	public void handleRequest(Database db, MolgenisRequest request) throws Exception
	{
		if (request.getString("__action").equals("upload"))
		{
			...
			KeyValueTuple kvtdataParticipant = new KeyValueTuple();
			...
			List<String> listOfAllFeatures = Arrays.asList("row_identifier", "isCollaborationRequired",
					...
					"enrolls_row_identifier", "enrollsRef", "pcs", "race", "specimenId", "specimenCount", "diagnosis");

			List<String> listOfEnrollsFeatures = Arrays.asList("pcs", "enrolls_row_identifier", "race", "specimenId",
			...
			// Close the observableFeature file since it is not used anymore
			observableFeature.close();
			...
			for (CollectionProtocol collectionProtocolFromJaxb : result.getProtocols().getCollectionProtocol())
			{
				...
				kvtdataSetCollectionProtocol.set("hasMatchedSpecimens", collectionProtocolFromJaxb.getMakesAvailable()
				...
				kvtdataSetCollectionProtocol.set("hasExposureHistory", collectionProtocolFromJaxb.getMakesAvailable()
				...
				kvtdataSetCollectionProtocol.set("lastName", collectionProtocolFromJaxb.getIsAssignedTo().getLastName()
						.toString());

				kvtdataSetCollectionProtocol.set("firstName", collectionProtocolFromJaxb.getIsAssignedTo()
						.getFirstName().toString());

				kvtdataSetCollectionProtocol.set("fullName", collectionProtocolFromJaxb.getIsAssignedTo().getFullName()
						.toString());
				...
				kvtdataSetCollectionProtocol.set("state", collectionProtocolFromJaxb.getIsAssignedTo().getIsLocatedAt()
						.getState().toString());
				...
				kvtdataSetCollectionProtocol.set("city", collectionProtocolFromJaxb.getIsAssignedTo().getIsLocatedAt()
						.getCity().toString());
				...
				// FIXME: this is what you WANT to do: value is the link to the
				// dataset
						...
						.getParticipantCollectionSummary())
				{
					...
					{
						dataParticipant.close();
						throw new Exception("More than one race found for PCS " + pcsummary.getId());
					}

					kvtdataParticipant.set("enrolls_row_identifier", "enrolls_row_identifier" + collectionIdentifier);
					...
					{
						dataParticipant.close();
						throw new Exception("More than one SpecimenCollectionSummary found for PCS "
								+ pcsummary.getId());

					}

					kvtdataParticipant.set("specimenId", pcsummary.getProvides().getSpecimenCollectionSummary().get(0)
							.getId().toString());
					kvtdataParticipant
							.set("specimenCount", pcsummary.getProvides().getSpecimenCollectionSummary().get(0)
			...
			dataSetCollectionProtocol.write(kvtdataSetCollectionProtocol);
			dataSetCollectionProtocol.close();
		}
	}
