...
public class PrintLabelPlugin extends EasyPluginController {
	 ...
	 */
	private void handlePrintRequest(Database db, Tuple request)
		...
		List<String> investigationNames = cs
				.getAllUserInvestigationNames(userName);
		List<ObservationTarget> individualList = getIndividualsFromUi(db,
				request);
		...
		for (ObservationTarget ind : individualList) {
			...
			List<ObservedValue> valueList = cs
					.getObservedValuesByTargetAndFeatures(ind.getName(),
							measurementList, investigationNames, ownInvName);
	 ...
	 * @throws MatrixException
	 */
	private List<ObservationTarget> getIndividualsFromUi(Database db,
			Tuple request) throws DatabaseException, ParseException,
			MatrixException {
	 ...
	 * @throws ParseException
	 */
	private List<String> getMeasurementsFromUi(Tuple request)
	...
	public void reload(Database db) {
		...
		try {
			...
			e.printStackTrace();
			this.setMessages(new ScreenMessage("Something went wrong: "
					+ e.getMessage(), false));
		}
	}
	...
	public void makeTargetsSelect(Database db) throws Exception {
		List<String> investigationNames = cs.getAllUserInvestigationNames(db
	...
	private void makeFeaturesSelect(Database db) {
		...
		try {
			List<String> investigationNames = cs
					.getAllUserInvestigationNames(db.getLogin().getUserName());
			for (Measurement feature : cs.getAllMeasurementsSorted(
					Measurement.NAME, "ASC", investigationNames)) {
				features.addOption(feature.getName(), feature.getName());
			}
		} catch (Exception e) {
			this.setMessages(new ScreenMessage(
					"An error occurred while retrieving features from the database",
					false));
		}
		panel.add(features);
	}
