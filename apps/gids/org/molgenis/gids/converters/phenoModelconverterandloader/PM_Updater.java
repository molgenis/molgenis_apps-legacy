...
public class PM_Updater
{
	...
	{
		List<ObservedValue> observedValueList = db.find(ObservedValue.class, new QueryRule(
				ObservedValue.INVESTIGATION_NAME, Operator.EQUALS, investigation));
		List<GidsSample> sampleGIDS = db.find(GidsSample.class, new QueryRule(ObservedValue.INVESTIGATION_NAME,
				Operator.EQUALS, investigation));
		/*
		 * if(db.query(Investigation.class).eq(Investigation.NAME,
		 * investigation).count() !=0){ listIndividuals =
		 * db.find(Individual.class, new
		 * QueryRule(Individual.INVESTIGATION_NAME, Operator.EQUALS,
		 * investigation)); }
		 */
