 ...
 * user <li>EditIndividualModel holds application state and business logic on
 * top of domain model. Get it via this.getModel()/setModel(..) <li>
 * EditIndividualView holds the template to show the layout. Get/set it via
	...
	public Show handleRequest(Database db, Tuple request, OutputStream out) throws HandleRequestDelegationException
	{
		...
		try
		{
			...
			if (action.equals("goToIndiv"))
			{
				...
				getModel().setListIndiv(
						db.find(Individual.class, new QueryRule(Individual.INVESTIGATION_NAME, Operator.EQUALS,
								getModel().chosenInv)));
