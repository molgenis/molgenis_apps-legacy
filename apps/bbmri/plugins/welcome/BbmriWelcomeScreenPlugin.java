...
public class BbmriWelcomeScreenPlugin<E extends Entity> extends PluginModel<E>
{
	...
	// todo : remove functionality to reload where database is available .
	// currently not working , check why.
	public BbmriWelcomeScreenPlugin(String name, ScreenController<?> parent)
	{
		...
		try
		{
			...
			welcomeList = db.find(Welcome.class, new QueryRule(Welcome.STATUS, Operator.EQUALS, "new"), new QueryRule(
					Operator.SORTASC, Welcome.WELCOMEDATETIME)); // get latest
																	// new

			// String query =
			// "SELECT * FROM welcome where status=\"new\" ORDER BY welcomeDatetime DESC limit 1;";
			...
			if (welcomeList.isEmpty())
			{
				...
				// String helpLink =
				// "molgenis.do?__target=main&select=BbmriHelp";
				...
				// ;
				// "<p>To apply for inclusion of your biobank in this catalogue, please <a href=\" "+
				// contactLink + "\">contact the BBMRI-NL office</a>.</p>" +
				// "<p>To find your way around the application, you might want to check out the <a href=\" "+
				// helpLink + "\">User manual</a>.</p>" +
				// "<p>If you have any questions or remarks, please do not hesitate to <a href=\" "+contactLink
				// + "\">contact us</a>.</p>") ;
