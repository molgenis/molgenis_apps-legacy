...
/**
 * Expands the query by adding synonyms and children to initial query, using
 * Boolean OR (not necessary, but more convenient to look through the query),
 * expansion terms are weighted less than initial query terms
 * 
 * @param query_terms
 *            - chunks: all possible combinations of query terms(subsequent)
 * @param init_query
 *            - list of query terms, to which the expansion terms are then added
 * @param boost_factor
 *            - the weight of expansion terms
 * @param spec_symbols
 *            - supported Boolean search operators (& = AND, | = OR, both
...
public class OntocatQueryExpansion_lucene
{
	...
	public List<String> parseQuery(String query)
	{
		...
		query = query.replaceAll(ignore, " ").trim();
		/** remove punctuation */
		...
		// * splitting by ' ' and by spec_symbols, leaving phrases in "" as
		// single unit phrases */
		while (i < len)
		{
			...
			{
				if (i != first_letter) words.add(query.substring(first_letter, i));
				char[] tmp = new char[1];
				tmp[0] = ch;
		...
		/**
		 * chunking the query into all possible n-grams, skipping the Boolean
		 * operators
		 */
		int size = words.size();
		for (int x = 0; x < size; x++)
		{
			...
			if (!spec_symbols.contains(cur)) tmp.add(cur);
			else
			{
				result.addAll(chunk(tmp));
				tmp.clear();
			}
			if ((!tmp.isEmpty()) && (x == size - 1)) result.addAll(chunk(tmp));
		}
		query_terms = result;
		return result;
	}
	...
	/**
	 * Query expansion. Changes init_query by adding expansion terms after found
	 * terms and joining found phrase terms hrase - current part of the query
	 * found_terms - terms found in ontologies
	 * searcher.SearchIndexOntocat(String phrase, List<Strings> ontologies)
	 * searches the phrase in index files of ontologies, returns
	 * (term:syn1;syn2;...;child1;child2;...) expansion - synonyms + children
	...
	{
		/*
		 * phrase - current part of the query found_terms - terms found in
		 * ontologies searcher.SearchIndexOntocat(String phrase, List<Strings>
		 * ontologies) searches the phrase in index files of ontologies, returns
		 * (term:syn1;syn2;...;child1;child2;...) expansion - synonyms +
		 * children found all - found_terms + expansion
		 */
		...
		while (z < query_terms.size())
		{
			...
			if (!found_terms.isEmpty())
			{
				...
				if (!found_all.contains(found_terms.get(0))) expansion.add(found_terms.get(0));
				...
				/**
				 * replacing the words in init_query, corresponding to the
				 * phrase, with the expanded phrase (found_all)
				 */
				...
				{
					if (w.equals(first_word)) first_index = init_query.indexOf(w);
					if (w.equals(phrase))
					{
						first_index = init_query.indexOf(w);
						last_index = init_query.indexOf(w);
					}
					if (w.equals(last_word)) last_index = init_query.indexOf(w);
				...
				if (!init_query.isEmpty()) init_query.addAll(first_index, found_all);

				else
					init_query.addAll(found_all);

				/**
				 * replacing phrases, containing words from the found phrase,
				 * from query terms (to avoid duplicate expansions, to reduce
		...
		for (String s : init_query)
		{
			if (i + 1 < size)
			{
				...
				if ((parsed.contains(s)) || (spec_symbols.contains(s)))
				{
					 ...
					 */
					if (s.split(" ").length == 1) res_query += s + " ";
					else
						res_query += "\"" + s + "\"" + " ";
					if ((!parsed.contains(next)) && (!spec_symbols.contains(next))) res_query += "OR (";
			...
			{
				if (size > 1) if ((parsed.contains(s)) || (spec_symbols.contains(s))) if (s.split(" ").length == 1) res_query += s;
				else
					res_query += "\"" + s + "\"";
				else
					res_query += "" + s + "^" + boost_factor + ")";
				else if (s.split(" ").length == 1) res_query += s;
				else
					res_query += "\"" + s + "\"";
				break;
			}
	...
	{
		// String query =
		// "never asthma AND (\"cystic lung disease\" OR (Parkinson Disease))";
