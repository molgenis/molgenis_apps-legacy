...
/**
 * Indexes all ontologies specified in ontologyNamesMap. The ontologies should
 * be downloaded on the computer in ONTOLOGIES_DIRECTORY Searches through the
 * index
 * 
 * @param LUCENE_ONTOINDEX_DIRECTORY
 *            - an empty directory to store the index
 * @param ONTOLOGIES_DIRECTORY
 *            - the directory, where the ontologies are stored
 */
...
public class OntoCatIndexPlugin extends PluginModel<org.molgenis.util.Entity>
{
	...
	//
	// static final String LUCENE_ONTOINDEX_DIRECTORY =
	// "/Users/despoina/Documents/molgenis4phenotypeWorkspace/molgenis4phenotype/OntocatIndex";
	// static final String ONTOLOGIES_DIRECTORY =
	// "/Users/despoina/Documents/workspace/biobank_search/ontologies/";
		...
		if ("SearchOntocatLuceneIndex".equals(request.getAction()))
		{
			...
			if (!this.DirectoryhasContents(OntocatIndexDir))
			{
				try
				{
					...
					this.setStatus("<h4> Starting search for  " + request.getString("InputToken")
							+ "  in Ontocat index.</h4> ");
	...
	public void DeleteOntocatIndex()
	{
		...
		// this.setStatus("<h4>Produces from DeleteLuceneIndex "+
		// LC.getINDX()+"</h4>");
		...
		try
		{
			...
			if (hits.length > 0)
			{
				...
				for (int i = 0; i < hits.length; i++)
				{
					...
					// say("label: " +
					// document.getField("ontologyLabel").stringValue());
				}
