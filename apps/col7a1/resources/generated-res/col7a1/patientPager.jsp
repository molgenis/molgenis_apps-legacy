...
<display:table name="patientSummaryVOs" pagesize="20" export="true"
	sort="list" class="listtable" id="current">
	<display:setProperty name="paging.banner.full">
		<span class="pagelinks"><a href="{1}">First</a> <a href="{2}">Prev</a>
			{0} <a href="{3}">Next</a> <a href="{4}">Last</a></span>
	</display:setProperty>
	<display:setProperty name="paging.banner.first">
		<span class="pagelinks">{0} <a href="{3}">Next</a> <a
			href="{4}">Last</a>
		</span>
	</display:setProperty>
	...
	<display:column title="No.">
		<c:out value="${current_rowNum}" />
	</display:column>
	<display:column media="html" property="patientIdentifier"
		title="Patient ID" sortable="true" headerClass="sortable"
		href="molgenis.do?__target=SearchPlugin&__action=showPatient&pid=#results"
		paramId="pid" paramProperty="patientIdentifier" />
	<display:column media="csv excel pdf" property="patientIdentifier"
		title="Patient ID" escapeXml="false" />
	<display:column title="Phenotype" sortable="true">
		<c:out value="${current.phenotypeMajor}" />
		<c:if test="${fn:length(current.phenotypeSub) > 1}">, <c:out
				value="${current.phenotypeSub}" />
		</c:if>
	</display:column>
	<display:column media="html" title="Mutation" class="nowrap"
		sortable="false">
	...
	<display:column media="html" title="cDNA change">
		<div class="unwrapped">
			<c:url var="url"
				value="molgenis.do?__target=SearchPlugin&__action=showMutation&mid=${current.variantDTOList[0].identifier}#results" />
			<a href="<c:out value="${url}"/>"><c:out
					value="${current.variantDTOList[0].cdnaNotation}" /></a>
		</div>
		<div class="unwrapped">
			<c:choose>
				<c:when test="${fn:length(current.variantDTOList) > 1}">
					<c:url var="url"
						value="molgenis.do?__target=SearchPlugin&__action=showMutation&mid=${current.variantDTOList[1].identifier}#results" />
					<a href="<c:out value="${url}"/>"><c:out
							value="${current.variantDTOList[1].cdnaNotation}" /></a>
				</c:when>
				<c:otherwise>
					<c:choose>
						<c:when
							test="${fn:contains(current.variantDTOList[0].inheritance, 'dominant')}">
			NA
		</c:when>
						<c:otherwise>
			Unknown
	...
	<display:column media="html" title="Exon">
		<div class="unwrapped">
			<c:url var="url"
				value="molgenis.do?__target=SearchPlugin&__action=showExon&exon_id=${current.variantDTOList[0].exonId}#results" />
			<a href="<c:out value="${url}"/>"><c:out
					value="${current.variantDTOList[0].exonName}" /></a>
		</div>
		<div class="unwrapped">
			<c:choose>
				<c:when test="${fn:length(current.variantDTOList) > 1}">
					<c:url var="url"
						value="molgenis.do?__target=SearchPlugin&__action=showExon&exon_id=${current.variantDTOList[1].exonId}#results" />
					<a href="<c:out value="${url}"/>"><c:out
							value="${current.variantDTOList[1].exonName}" /></a>
				</c:when>
				<c:otherwise>
	&nbsp;
	</c:otherwise>
	...
	<display:column media="csv excel pdf" title="cDNA change 2">
		<c:if test="${fn:length(current.variantDTOList) > 1}">
			<c:out value="${current.variantDTOList[1].cdnaNotation}"
				escapeXml="false" />
		</c:if>
	</display:column>
	...
	<display:column media="csv excel pdf" title="Exon/Intron 2">
		<c:if test="${fn:length(current.variantDTOList) > 1}">
			<c:out value="${current.variantDTOList[1].exonName}"
				escapeXml="false" />
		</c:if>
	</display:column>
