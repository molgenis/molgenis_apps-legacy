...
public class developingAlgorithm extends PluginModel<Entity> {
	...
	public void handleRequest(Database db, Tuple request) throws Exception {
		...
		if (request.getAction().equals("generateAlgorithm")) {
			...
			String ontologyFileName = request
					.getString("ontologyFileForAlgorithm");
			...
			if (db.find(
				...
				measurementsInStudy = db.find(Measurement.class, new QueryRule(
			...
			messageForAlgorithm = "The algorithms for " + validationStudyName
	...
	private void generateAlgorithm(Database db) throws DatabaseException {
		...
		if (db.find(
				MappingMeasurement.class,
				new QueryRule(MappingMeasurement.INVESTIGATION_NAME,
						Operator.EQUALS, validationStudyName)).size() > 0) {

			List<MappingMeasurement> listOfMappings = db.find(
					MappingMeasurement.class, new QueryRule(
							MappingMeasurement.INVESTIGATION_NAME,
							Operator.EQUALS, validationStudyName));

			HashMap<String, String> variableToScript = testModel
					.getListOfVariable();
			...
			for (MappingMeasurement mapping : listOfMappings) {
				...
				if (owlFunction.getAnnotation(mappedParameter, "Variable_Type")
						.size() > 0) {
					variableType = owlFunction.getAnnotation(mappedParameter,
							"Variable_Type").get(0);
				}
				...
				if (variableFormula.containsKey(mappedParameter)
					...
					formula = formula.replaceAll(mappedParameter.toLowerCase(),
							substitute);

					System.out.println(formula);

					List<String> composites = owlFunction
							.getComposites(mappedParameter);
					...
					for (String variable : featureNames) {
						...
						Measurement m = db.find(
								...
								new QueryRule(Measurement.NAME,
										Operator.EQUALS, variable)).get(0);

						String description = m.getDescription();

						List<String> tokensForDataItem = model.createNGrams(
								description.toLowerCase().trim(), false);
						...
						for (String buildingBlock : composites) {
							...
							List<String> synonyms = owlFunction
									.getSynonyms(buildingBlock);

							synonyms.add(buildingBlock);

							for (String eachMatchingString : synonyms) {

								List<String> tokensForBuildingBlock = model
										.createNGrams(eachMatchingString
												.toLowerCase().trim(), false);

								double similarity = model.calculateScore(
										tokensForBuildingBlock,
										tokensForDataItem);

								if (maxSimilarity < similarity) {
									maxSimilarity = similarity;
									matchedItem = variable;
									featureToBuildingBlock.put(matchedItem,
											buildingBlock);
								}
							}
						}
					...
					for (Entry<String, String> eachEntry : featureToBuildingBlock
							.entrySet()) {

						Pattern pattern = Pattern.compile(eachEntry.getValue()
								.toLowerCase());
						...
						formula = formula.replaceAll(
								"\"" + eachEntry.getValue().toLowerCase()
										+ "\"",
								"as.numeric(dataSet[rowIndex,\""
										+ eachEntry.getKey() + "\"])");
				...
				if (variableType.equalsIgnoreCase("Categorical")) {
					...
					if (featureNames.size() == 1) {
						...
						List<String> listOfCode = owlFunction.getAnnotation(
								mappedParameter, "codeString");
						...
						if (listOfCode.size() > 0) {
							...
							if (db.find(
								...
								Category category = db.find(
										Category.class,
										new QueryRule(Category.NAME,
												Operator.IN, listOfCode))
										.get(0);

								codeValue = Integer.parseInt(category
										.getCode_String().split("=")[0].trim());

							} else {

								List<String> synonymsForCodeString = owlFunction
										.getSynonyms(codeString);

								if (synonymsForCodeString.size() > 0) {
									List<Category> categories = db.find(
											Category.class, new QueryRule(
													Category.NAME, Operator.IN,
													synonymsForCodeString));

									if (categories.size() > 0)
										codeValue = Integer.parseInt(categories
												.get(0).getCode_String()
												.split("=")[0].trim());
								}
							}

							script += "if(as.numeric(dataSet[rowIndex, \""
									+ featureNames.get(0) + "\"]) == "
									+ codeValue + "){\n" + "\t"
									+ derivedParameter + " = 1;\n" + "}";

						} else {
							script += derivedParameter
									+ " = as.numeric(dataSet[rowIndex, \""
									+ featureNames.get(0) + "\"]);";
						}
