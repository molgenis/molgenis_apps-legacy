...
public class LoaderUtils
{
	...
	public static List<Measurement> getMeasurementsByInvestigationId(final int investigationId, EntityManager em,
			final String schemaName, final String tableName) throws Exception
	{
		Investigation investigation = em
				.createQuery("SELECT i FROM Investigation i WHERE i.id = :id", Investigation.class)
				.setParameter("id", investigationId).getSingleResult();
		return getMeasurements(em, investigation, schemaName, tableName);
	}

	public static List<Measurement> getMeasurements(EntityManager em, Investigation investigation, String schemaName,
			String tableName) throws Exception
	{
		...
		for (int i = 1; i <= rsmd.getColumnCount(); ++i)
		{
			...
			Measurement m = em
					.createQuery(
							"SELECT m FROM Measurement m WHERE m.name = :name AND m.investigation = :investigation",
							Measurement.class).setParameter("name", columnName)
					.setParameter("investigation", investigation).getSingleResult();
			measurements.add(m);
		}
		return measurements;
	}
