...
<display:table name="patientSummaryVOs" pagesize="20" export="true"
	...
	<display:column media="html" title="cDNA change" sortable="true"
		headerClass="sortable" sortProperty="gdnaStart">
		<c:forEach var="variantDTO" items="${current.variantDTOList}">
			<c:url var="url"
				value="molgenis.do?__target=SearchPlugin&__action=showMutation&mid=${variantDTO.identifier}#results" />
			<a href="<c:out value="${url}"/>"><c:out
					value="${variantDTO.cdnaNotation}" /></a>
			<br>
		</c:forEach>
	</display:column>
	<display:column media="html" title="Protein change" sortable="true"
		headerClass="sortable" sortProperty="gdnaStart">
		<c:forEach var="variantDTO" items="${current.variantDTOList}">
			<c:out value="${variantDTO.aaNotation}" />
			<br>
		</c:forEach>
	</display:column>
	<display:column media="html" title="Exon" sortable="true"
		headerClass="sortable">
		<c:forEach var="variantDTO" items="${current.variantDTOList}">
			<c:url var="url"
				value="molgenis.do?__target=SearchPlugin&__action=showExon&exon_id=${variantDTO.exonId}#results" />
			<a href="<c:out value="${url}"/>"><c:out
					value="${variantDTO.exonName}" /></a>
			<br>
		</c:forEach>
	</display:column>
	<display:column media="html" title="Consequence" sortable="true"
		headerClass="sortable">
		<c:forEach var="variantDTO" items="${current.variantDTOList}">
			<c:out value="${variantDTO.observedValue}" />
			<br>
		</c:forEach>
	</display:column>
				...
				<c:forEach var="publicationDTO"
					...
					<a href="${current.pubmedURL}${publicationDTO.pubmedId}"
						title="${publicationDTO.title}" target="_new"><c:out
							value="PM:${publicationDTO.pubmedId}" /></a>
					<br />
				</c:forEach>
			</c:when>
			<c:otherwise>
