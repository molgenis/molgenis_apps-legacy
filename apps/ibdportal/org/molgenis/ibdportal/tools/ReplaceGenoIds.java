...
public class ReplaceGenoIds
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		{
			throw new Exception(
					"You must supply 3 arguments: FAM file to be read from, CSV file for linking pheno to geno IDs, FAM file to be created");
		}
		...
		{
			throw new Exception("FAM file name '" + famFileName
					+ "' does not end with '.fam', are you sure it is a FAM file?");
		}
		...
		{
			throw new Exception("CSV file name '" + csvFileName
					+ "' does not end with '.csv', are you sure it is a CSV file?");
		}
		...
		while ((line = reader.readLine()) != null)
		{
			...
			// System.out.println("Added key = " + lineParts[1] + " value = " +
			// lineParts[0] + " to ID hashmap");
		}
		reader.close();
		...
		while ((line = reader.readLine()) != null)
		{
			String[] lineParts = line.split("\\s"); // FAM file contains 6
													// items, 2nd is ID
			String genoId = lineParts[1];
			String phenoId = idMap.get(genoId);
			if (phenoId != null)
			{
				// System.out.println("Replacing " + genoId + " with " + phenoId
				// + " on line " + lineNr);
				output.write(line.replace(genoId, phenoId) + "\n");
			}
			else
			{
				System.out.println("Keeping geno ID '" + genoId + "' on line " + lineNr
						+ " because it's not listed in your CSV file");
				output.write(line + "\n");
			}
			lineNr++;
		}
