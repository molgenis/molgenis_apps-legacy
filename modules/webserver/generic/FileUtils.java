 ...
 * 
 * This class contains generic function related to Files bugs: none found<br>
 */
public class FileUtils
{
	...
	static void writeInJar(JarOutputStream zos, File[] files, String path, boolean verbose) throws IOException
	{
		...
		for (int i = 0; i < files.length; i++)
		{
			...
			if (file.isDirectory())
			{
				...
				writeInJar(zos, file.listFiles(), path + ((!path.equals("")) ? File.separator : "") + file.getName(),
						verbose);
			}
			else
			{
	...
	static void writejarEntry(JarInputStream jarfile, ZipEntry entry, String path, boolean verbose) throws IOException
	{
		...
		if (verbose) System.out.println("Uncompressed: " + entry.getName() + " size: " + sumcount
				+ " with buffersize: " + buffer);
		dest.flush();
		dest.close();
	}
