...
public class GffService implements Serializable
{
	...
	public String exportExons() throws DatabaseException
	{
		...
		b.append(String.format("chr%s\t.\tgene\t%d\t%d\t.\t%s\t.\tID=gene%s;Name=%s\n", chromosome.getName(),
				gene.getBpEnd(), gene.getBpStart(), ("F".equals(gene.getOrientation()) ? "+" : "-"), gene.getId(),
				gene.getName()));

		b.append(String.format("chr%s\texonerate\tmRNA\t%d\t%d\t.\t%s\t.\tID=mRNA%s;Parent=gene%s\n", chromosome
				.getName(), gene.getBpEnd(), gene.getBpStart(), ("F".equals(gene.getOrientation()) ? "+" : "-"), gene
				.getId(), gene.getId()));

		List<Exon> exonList = this.db.query(Exon.class).equals(Exon.GENE, gene.getId()).sortASC(Exon.GDNA_POSITION)
				.find();

		for (Exon exon : exonList)
		{
			b.append(String.format("chr%s\texonerate\t%s\t%d\t%d\t.\t%s\t.\tID=%s;Parent=mRNA%s;Name=%s\n", chromosome
					.getName(), (exon.getIsIntron() ? (exon.getName().endsWith("UTR") ? "UTR" : "intron") : "exon"),
					exon.getGdna_Position() - exon.getLength() + 1, exon.getGdna_Position(), ("F".equals(gene
							.getOrientation()) ? "+" : "-"), exon.getId(), gene.getId(), exon.getName()));
		}
