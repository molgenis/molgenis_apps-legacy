...
public class MutationService implements Serializable
{
	...
	protected List<Mutation> _findMutations(MutationSearchCriteriaVO criteria) throws DatabaseException
	{
		if (this.db instanceof JDBCDatabase)
		{
			...
			if (criteria.getCdnaPosition() != null) query = query.equals(Mutation.CDNA_POSITION,
					criteria.getCdnaPosition());
			if (criteria.getCodonChangeNumber() != null) query = query.equals(Mutation.AA_POSITION,
					criteria.getCodonChangeNumber());
			if (StringUtils.length(criteria.getConsequence()) > 2) query = query.like(Mutation.CONSEQUENCE,
					criteria.getConsequence() + "%");
			if (criteria.getExonId() != null) query = query.equals(Mutation.EXON, criteria.getExonId());
			if (criteria.getExonName() != null) query = query
					.equals(Mutation.EXON_NAME, "Exon " + criteria.getExonName()).or()
					.equals(Mutation.EXON_NAME, "IVS" + criteria.getExonName());
			if (criteria.getExonNumber() != null) query = query
					.equals(Mutation.EXON_NAME, "Exon " + criteria.getExonNumber()).or()
					.equals(Mutation.EXON_NAME, "IVS" + criteria.getExonNumber());
			if (criteria.getMid() != null) query = query.equals(Mutation.IDENTIFIER, criteria.getMid());
			if (StringUtils.length(criteria.getInheritance()) > 2) query = query.like(Mutation.INHERITANCE,
					criteria.getInheritance() + "%");
			if (criteria.getMutationId() != null) query = query.equals(Mutation.ID, criteria.getMutationId());
			if (StringUtils.length(criteria.getPhenotypeName()) > 2)
			{
				...
				List<ObservedValue> phenotypes = this.db.query(ObservedValue.class)
						.equals(ObservedValue.VALUE, criteria.getPhenotypeName()).find();
				for (ObservedValue phenotype : phenotypes)
				{
					...
					if (mutationIds.size() > 0) query = query.in(Mutation.ID, mutationIds);
				}
			}
			// if (StringUtils.length(criteria.getPhenotypeName()) > 2)
			// {
			...
			// .like(MutationPhenotype.NAME, criteria.getPhenotypeName() +
			// "%").or()
			// .like(MutationPhenotype.MAJORTYPE, criteria.getPhenotypeName() +
			// "%").or()
			...
			// this.db.query(Patient.class).in(Patient.PHENOTYPE,
			// phenotypeIds).find();
			...
			{
				// TODO: add proper join capability
				List<Patient> patients = this.db.query(Patient.class).equals(Patient.IDENTIFIER, criteria.getPid())
						.find();
				List<Integer> mutationIds = new ArrayList<Integer>();
				for (Patient patient : patients)
				{
			...
			if (criteria.getProteinDomainId() != null)
			{
				...
				List<Exon> exons = this.db.query(Exon.class).equals(Exon.PROTEINDOMAIN, criteria.getProteinDomainId())
						.find();
				List<Integer> exonIds = new ArrayList<Integer>();
				for (Exon exon : exons)
					exonIds.add(exon.getId());
				if (exonIds.size() > 0) query = query.in(Mutation.EXON, exonIds);
			}
			if (StringUtils.length(criteria.getPublication()) > 2)
			{
				List<Publication> publications = this.db.query(Publication.class)
						.like(Publication.NAME, criteria.getPublication() + "%").or()
						.like(Publication.TITLE, "%" + criteria.getPublication() + "%").find();
				List<Integer> publicationIds = new ArrayList<Integer>();
				for (Publication publication : publications)
					publicationIds.add(publication.getId());
				if (publicationIds.size() > 0)
				{
					List<Patient> patients = this.db.query(Patient.class).in(Patient.PATIENTREFERENCES, publicationIds)
							.find();
					List<Integer> mutationIds = new ArrayList<Integer>();
					for (Patient patient : patients)
					{
			...
			if (criteria.getReportedAsSNP() != null) query = query.equals(Mutation.REPORTEDSNP,
					criteria.getReportedAsSNP());
			if (StringUtils.length(criteria.getType()) > 2) query = query
					.like(Mutation.TYPE_, criteria.getType() + "%");
			if (StringUtils.length(criteria.getVariation()) > 0) query = query
					.equals(Mutation.CDNA_NOTATION, criteria.getVariation()).or()
					.equals(Mutation.CDNA_NOTATION, "c." + criteria.getVariation()).or()
					.equals(Mutation.AA_NOTATION, criteria.getVariation()).or()
					.equals(Mutation.AA_NOTATION, "p." + criteria.getVariation());

			if (query.getRules().length > 0) return query.sortASC(Mutation.IDENTIFIER).find();
			else
		...
		else if (this.db instanceof JpaDatabase)
		{
			...
			if (criteria.getCdnaPosition() != null) mutationCriteria.add(cb.equal(mutation.get("cdna_position"),
					criteria.getCdnaPosition()));
			if (criteria.getCodonChangeNumber() != null) mutationCriteria.add(cb.equal(mutation.get("aa_position"),
					criteria.getCodonChangeNumber()));
			if (StringUtils.length(criteria.getConsequence()) > 2) mutationCriteria.add(cb.like(
					mutation.<String> get("consequence"), criteria.getConsequence() + "%"));
			if (criteria.getExonId() != null || criteria.getExonName() != null || criteria.getExonNumber() != null
					|| criteria.getProteinDomainId() != null)
			{
				Join<Mutation, Exon> exon = mutation.join("exon", JoinType.LEFT);

				if (criteria.getExonId() != null) mutationCriteria.add(cb.equal(exon.get("id"), criteria.getExonId()));
				if (criteria.getExonName() != null) mutationCriteria.add(cb.or(
						cb.like(exon.<String> get("name"), "Exon " + criteria.getExonName()),
						cb.like(exon.<String> get("name"), "IVS" + criteria.getExonName())));
				if (criteria.getExonNumber() != null) mutationCriteria.add(cb.or(
						cb.like(exon.<String> get("name"), "Exon " + criteria.getExonNumber()),
						cb.like(exon.<String> get("name"), "IVS" + criteria.getExonNumber())));
			...
			if (criteria.getMid() != null) mutationCriteria
					.add(cb.equal(mutation.get("identifier"), criteria.getMid()));
			if (StringUtils.length(criteria.getInheritance()) > 2) mutationCriteria.add(cb.like(
					mutation.<String> get("inheritance"), criteria.getInheritance() + "%"));
			if (criteria.getMutationId() != null) mutationCriteria.add(cb.equal(mutation.get("id"),
					criteria.getMutationId()));
			if (criteria.getPid() != null || criteria.getPhenotypeId() != null
					|| StringUtils.length(criteria.getPhenotypeName()) > 2
					|| StringUtils.length(criteria.getPublication()) > 2)
			{
				Join<Mutation, Patient> patient = mutation.join("mutationsCollection", JoinType.LEFT);

				if (criteria.getPid() != null) mutationCriteria.add(cb.equal(patient.get("identifier"),
						criteria.getPid()));
				...
				if (StringUtils.length(criteria.getPhenotypeName()) > 2)
				{
					Join<Patient, ObservedValue> observedValues = patient.join("targetObservedValueCollection",
							JoinType.LEFT);
					Join<ObservedValue, ObservableFeature> observableFeatures = observedValues.join("feature",
							JoinType.LEFT);

					mutationCriteria.add(cb.or(
							cb.and(cb.equal(observableFeatures.get("name"), "Phenotype"),
									cb.equal(observedValues.get("value"), criteria.getPhenotypeName())),
							cb.and(cb.equal(observableFeatures.get("name"), criteria.getPhenotypeName()),
									cb.equal(observedValues.get("value"), "yes"))));
				}
				...
				if (StringUtils.length(criteria.getPublication()) > 2)
				{
					...
					mutationCriteria.add(cb.or(
							cb.like(publication.<String> get("name"), "%" + criteria.getPublication() + "%"),
							cb.like(publication.<String> get("title"), "%" + criteria.getPublication() + "%")));
				}
			}
			if (criteria.getReportedAsSNP() != null) mutationCriteria.add(cb.equal(mutation.get("reportedsnp"),
					criteria.getMutationId()));
			if (StringUtils.length(criteria.getType()) > 2) mutationCriteria.add(cb.like(
					mutation.<String> get("type_"), criteria.getType() + "%"));
			if (StringUtils.length(criteria.getVariation()) > 0) mutationCriteria.add(cb.or(
					cb.equal(mutation.get("cdna_notation"), criteria.getVariation()),
					cb.equal(mutation.<String> get("cdna_notation"), "c." + criteria.getVariation()),
					cb.equal(mutation.get("aa_notation"), criteria.getVariation()),
					cb.equal(mutation.get("cdna_notation"), "p." + criteria.getVariation())));
	...
	{
		if (this.cache.containsKey(mutation.getId())) return this.cache.get(mutation.getId());
		...
		if (StringUtils.isNotEmpty(mutation.getAa_Notation())) mutationSummaryVO.setNiceNotation(mutation
				.getCdna_Notation() + " (" + mutation.getAa_Notation() + ")");
		else
		...
		List<ProteinDomain> proteinDomains = this.db.query(ProteinDomain.class)
				.in(ProteinDomain.ID, exon.getProteinDomain_Id()).find();
		...
		for (Patient patient : patients)
		{
			...
			List<ObservedValue> phenotypes = this.db.query(ObservedValue.class)
					.equals(ObservedValue.FEATURE, features.get(0).getId())
			...
			// String phenotypeName = phenotype.getMajortype() +
			// (StringUtils.isNotEmpty(phenotype.getSubtype()) ? ", " +
			// phenotype.getSubtype() : "");
			...
			List<Mutation> otherVariants = this.db.query(Mutation.class).in(Mutation.ID, patient.getMutations_Id())
					.find();
			Mutation remove = new Mutation();
			for (Mutation patientMutation : otherVariants)
			{
				if (mutation.getId().equals(patientMutation.getId())) remove = patientMutation;
			}
	...
	private MutationSummaryVO findPrevMutation(Mutation mutation) throws DatabaseException
	{
		List<Mutation> mutations = this.db.query(Mutation.class)
				.lt(Mutation.CDNA_POSITION, mutation.getCdna_Position()).sortDESC(Mutation.CDNA_POSITION).limit(1)
				.find();

		if (mutations.size() > 0) return this.toMutationSummaryVO(mutations.get(0));
		else
	...
	{
		List<Mutation> mutations = this.db.query(Mutation.class)
				.gt(Mutation.CDNA_POSITION, mutation.getCdna_Position()).sortASC(Mutation.CDNA_POSITION).limit(1)
				.find();
	...
	{
		return this.toMutationSummaryVO(this.db.query(Mutation.class).sortASC(Mutation.CDNA_POSITION).limit(1).find()
				.get(0));
	}
	...
	{
		return this.toMutationSummaryVO(this.db.query(Mutation.class).sortDESC(Mutation.CDNA_POSITION).limit(1).find()
				.get(0));
	}
	...
	{
		List<MutationSummaryVO> result = new ArrayList<MutationSummaryVO>();
		List<Mutation> positionMutations = this.db.query(Mutation.class)
				.equals(Mutation.CDNA_POSITION, mutationSummaryVO.getCdnaPosition()).find();
	...
	public List<MutationSummaryVO> findCodonMutations(MutationSummaryVO mutationSummaryVO) throws DatabaseException
	{
		...
		{
			List<Mutation> codonMutations = this.db.query(Mutation.class)
					.equals(Mutation.AA_POSITION, mutationSummaryVO.getAaPosition()).find();
	...
	public List<String> getConsequences() throws SQLException, DatabaseException
	{
		if (this.db instanceof JDBCDatabase)
		{
			...
			ResultSet rs = ((JDBCDatabase) this.db).executeQuery("SELECT DISTINCT consequence FROM Mutation",
					(QueryRule[]) null);
			while (rs.next())
		...
		else if (this.db instanceof JpaDatabase)
		{
			javax.persistence.Query q = this.db.getEntityManager().createNativeQuery(
					"SELECT DISTINCT consequence FROM Mutation");
	...
	public List<String> getMutationTypes() throws SQLException, DatabaseException
	{
		if (this.db instanceof JDBCDatabase)
		{
			...
			ResultSet rs = ((JDBCDatabase) this.db).executeQuery("SELECT DISTINCT type_ FROM Mutation",
					(QueryRule[]) null);
			while (rs.next())
		...
		{
			javax.persistence.Query q = this.db.getEntityManager().createNativeQuery(
					"SELECT DISTINCT type_ FROM Mutation");
			return q.getResultList();
		}
		else
			throw new DatabaseException("Unsupported database mapper");
	...
	{
		if (mutation.getAa_Position() == null || mutation.getAa_Position() == 0
				|| StringUtils.isEmpty(mutation.getCodonchange())) return "";
	...
	public void assignValuesFromNotation(MutationUploadVO mutationUploadVO) throws DatabaseException, ParseException,
			RESyntaxException
	{
		...
		if (mutationUploadVO.getMutation().getCdna_Notation() != null)
		{
			...
			// RE reIndel = new
			// RE("^c.([\\d+-]+)([_]*)([\\d+-]*)del([ACGTacgt\\d]*)ins([ACGTacgt]*)$");
			...
			if (reIndel.match(cdnaNotation))
			{
				...
				if (position.length == 2) mutationUploadVO.getMutation().setLength(
						Integer.valueOf(position[1]) - Integer.valueOf(position[0]) + 1);
				else if (StringUtils.isNotEmpty(deletion)) if (StringUtils.isNumeric(deletion)) mutationUploadVO
						.getMutation().setLength(Integer.valueOf(deletion));
				else
			...
			else if (reDeletion.match(cdnaNotation))
			{
				...
				// mutationUploadVO.getMutation().setLength(Integer.valueOf(reDeletion.getParen(3))
				// - Integer.valueOf(reDeletion.getParen(1)) + 1);
			...
			else if (reDuplication.match(cdnaNotation))
			{
				...
				if (position.length == 2) mutationUploadVO.getMutation().setLength(
						Integer.valueOf(position[1]) - Integer.valueOf(position[0]) + 1);
				else if (StringUtils.isNotEmpty(duplication)) if (StringUtils.isNumeric(duplication)) mutationUploadVO
						.getMutation().setLength(Integer.valueOf(duplication));
				else
				...
				// mutationUploadVO.getMutation().setLength(Integer.valueOf(reDuplication.getParen(3))
				// - Integer.valueOf(reDuplication.getParen(1)) + 1);
	...
	{
		if (StringUtils.isEmpty(mutationUploadVO.getMutation().getMutationPosition())
				|| "0".equals(mutationUploadVO.getMutation().getMutationPosition())) return;

		MutationGene gene = this.db.query(MutationGene.class)
				.equals(MutationGene.NAME, mutationUploadVO.getGeneSymbol()).find().get(0);
		...
		// HashMap<String, String> aminoAcidHash =
		// this.mutationService.getAminoAcids();
		...
		if (mutationUploadVO.getMutation().getMutationPosition().indexOf("+") > -1
				|| mutationUploadVO.getMutation().getMutationPosition().indexOf("-") > -1) criteria.setIsIntron(true);
		else
		...
		// mutationUploadVO.getMutation().getPosition() + ", criteria==" +
		// criteria.toString());
		List<ExonSummaryVO> exonSummaryVOs = exonService.findExons(criteria);
		// logger.debug(">>> assignValues: nach findExons: exonSummaryVOs==" +
		// exonSummaryVOs.size());
		...
		mutationUploadVO.getMutation().setExon(exon); // this is crap, use
														// navigable objects
		System.out.println(">>> assignValuesFromPosition: mut.pos=="
				+ mutationUploadVO.getMutation().getMutationPosition() + ", exon==" + exon.getName());
		mutationUploadVO.getMutation().setCdna_Position(
				SequenceUtils.getCDNAPosition(mutationUploadVO.getMutation().getMutationPosition()));
		mutationUploadVO.getMutation().setGdna_Position(
				SequenceUtils.getGDNAPosition(mutationUploadVO.getMutation().getMutationPosition(), exon,
						gene.getOrientation()));
		...
		if ("F".equals(mutationUploadVO.getGeneOrientation())) mutationStart = mutationUploadVO.getMutation()
				.getGdna_Position() - mutationUploadVO.getGeneBpStart().intValue();
		else
			mutationStart = mutationUploadVO.getGeneBpStart().intValue()
					- mutationUploadVO.getMutation().getGdna_Position();

		if (mutationUploadVO.getMutation().getLength() == null) mutationUploadVO.getMutation().setLength(1); // default
																												// value
		...
		// mutationStart + 1).toUpperCase());
		// logger.debug("mutationStart==" + mutationStart + ".");
		// logger.debug("nt==" + nuclSequence.substring(mutationStart,
		// mutationStart + 1).toUpperCase() + ".");
		...
		else if (mutationUploadVO.getMutation().getEvent().equals("point mutation"))
		{
			...
			// this.mutationVO.setNt(nuclSequence.substring(mutationStart,
			// mutationEnd).toUpperCase());
			if (mutationUploadVO.getMutation().getNtchange() != null) mutSequence.replace(mutationStart, mutationEnd,
					mutationUploadVO.getMutation().getNtchange());
			mutationUploadVO.getMutation().setLength(mutationUploadVO.getMutation().getNtchange().length());
		}
		...
		else
		{
			...
			mutationUploadVO.setCodon(SequenceUtils.getCodonByPosition(splNuclSeq, mutTripletPos)); // StringUtils.substring(splNuclSeq,
																									// mutTripletPos,
																									// mutTripletPos
																									// +
																									// 3));
			mutationUploadVO.getMutation().setCodonchange(SequenceUtils.getCodonByPosition(splMutSeq, mutTripletPos)); // StringUtils.substring(splMutSeq,
																														// mutTripletPos,
																														// mutTripletPos
																														// +
																														// 3));
			mutationUploadVO.setAa(SequenceUtils.getCodonByPosition(trlNuclSeq, trlTripletPos)); // StringUtils.substring(trlNuclSeq,
																									// trlTripletPos,
																									// trlTripletPos
																									// +
																									// 3));
			mutationUploadVO.setAachange(SequenceUtils.getCodonByPosition(trlMutSeq, trlTripletPos)); // StringUtils.substring(trlMutSeq,
																										// trlTripletPos,
																										// trlTripletPos
																										// +
																										// 3));
		...
		// logger.debug(">>> assignValuesFromPosition: mutation==" +
		// mutationUploadVO.getMutation().toString());
		// logger.debug(">>> end of assignValues: nt==" +
		// mutationUploadVO.getNt());
	}
	...
	private void assignNt(MutationUploadVO mutationUploadVO, String nuclSequence, int mutationStart)
	{
		...
		// System.out.println(">>> assignNt: start: mutation==" +
		// this.getMutation());
		...
		// System.out.println(">>> assignNt: vor setNt, seqlen==" +
		// nuclSequence.length() + ", seq==" + nuclSequence);
	...
	private void assignConsequence(MutationUploadVO mutationUploadVO)
	{
		...
		else if (mutationUploadVO.getMutation().getAa_Notation().indexOf("fsX") > -1
				|| mutationUploadVO.getMutation().getAa_Notation().indexOf("Ter") > -1) mutationUploadVO.getMutation()
				.setConsequence("Premature termination codon");
		else if (mutationUploadVO.getMutation().getAa_Position() != null
				&& mutationUploadVO.getMutation().getAa_Position() == 1) mutationUploadVO.getMutation().setConsequence(
				"No initiation of transcription/translation");
	}

	private void assignType(MutationUploadVO mutationUploadVO)
	{
		if (mutationUploadVO.getExonIsIntron()) mutationUploadVO.getMutation().setType("splice-site mutation");
		else if (mutationUploadVO.getMutation().getEvent().equals("deletion")) if (mutationUploadVO.getMutation()
				.getLength() <= 20) if (mutationUploadVO.getMutation().getAa_Notation().indexOf("fsX") > -1) mutationUploadVO
				.getMutation().setType("small deletion frame-shift");
		else
			mutationUploadVO.getMutation().setType("small deletion in-frame");
		else if (mutationUploadVO.getMutation().getAa_Notation().indexOf("fsX") > -1) mutationUploadVO.getMutation()
				.setType("large deletion frame-shift");
		else
			mutationUploadVO.getMutation().setType("large deletion in-frame");
		else if (mutationUploadVO.getMutation().getEvent().equals("duplication")) if (mutationUploadVO.getMutation()
				.getLength() <= 20) if (mutationUploadVO.getMutation().getAa_Notation().indexOf("fsX") > -1) mutationUploadVO
				.getMutation().setType("small duplication frame-shift");
		else
		...
		else if (mutationUploadVO.getMutation().getEvent().equals("insertion")) if (mutationUploadVO.getMutation()
				.getLength() <= 20) if (mutationUploadVO.getMutation().getAa_Notation().indexOf("fsX") > -1) mutationUploadVO
				.getMutation().setType("small insertion frame-shift");
		else
	...
	{
		if (mutationUploadVO.getMutation().getLength() == null) return "";

		String notation = position;

		if (mutationUploadVO.getMutation().getEvent().equals("insertion")) notation += "_"
				+ SequenceUtils.getAddedPosition(position, 1);
		else if (mutationUploadVO.getMutation().getLength() > 1) notation += "_"
				+ SequenceUtils.getAddedPosition(position, mutationUploadVO.getMutation().getLength() - 1);

		if (mutationUploadVO.getMutation().getEvent().equals("deletion"))
		{
			notation += "del";
			if (mutationUploadVO.getMutation().getLength() <= 2) notation += mutationUploadVO.getNt();
		}
		else if (mutationUploadVO.getMutation().getEvent().equals("duplication"))
		{
			notation += "dup";
		...
		else if (mutationUploadVO.getMutation().getEvent().equals("insertion")) notation += "ins"
				+ mutationUploadVO.getMutation().getNtchange();
		else if (mutationUploadVO.getMutation().getEvent().equals("point mutation")) notation += mutationUploadVO
				.getNt() + ">" + mutationUploadVO.getMutation().getNtchange();
		else if (mutationUploadVO.getMutation().getEvent().equals("insertion/deletion"))
		{
			// deletion
			notation += "del";
	...
	{
		if (mutationUploadVO.getMutation().getLength() != null) mutationUploadVO.getMutation()
				.setGdna_Notation(
						"g."
								+ this.getNotation(mutationUploadVO, mutationUploadVO.getMutation().getGdna_Position()
										.toString()));
		else
	...
	{
		if (mutationUploadVO.getMutation().getLength() != null) mutationUploadVO.getMutation().setCdna_Notation(
				"c." + this.getNotation(mutationUploadVO, mutationUploadVO.getMutation().getMutationPosition()));
		else
	...
	{
		if (codonNum == 1) mutationUploadVO.getMutation().setAa_Notation("p.0");
		else if (codonNum > 1)
		{
			mutationUploadVO.getMutation().setAa_Notation(
					"p." + mutationUploadVO.getAa() + codonNum + mutationUploadVO.getAachange());
			if (mutationUploadVO.getMutation().getLength() % 3 != 0
					&& !mutationUploadVO.getMutation().getEvent().equals("point mutation"))
			{
				...
				if (terPos > -1) mutationUploadVO.getMutation().setAa_Notation(
						mutationUploadVO.getMutation().getAa_Notation() + "X" + (terPos - codonNum + 1)); // +
																											// " DEBUG: "
																											// +
																											// terPos
																											// +
