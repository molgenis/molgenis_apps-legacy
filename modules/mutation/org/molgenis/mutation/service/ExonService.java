...
public class ExonService implements Serializable
{
	...
	public List<ExonSummaryVO> findExons(ExonSearchCriteriaVO criteria) throws DatabaseException, RESyntaxException
	{
		if (this.db instanceof JDBCDatabase)
		{
			...
			// query = query.equals("id",
			// this.findExonIdByCdna_position(criteria.getCdnaPosition()));
			if (criteria.getExonId() != null) query = query.equals(Exon.ID, criteria.getExonId());
			...
			// query = query.equals("id",
			// this.findExonIdByGdna_position(criteria.getGdnaPosition()));
			if (criteria.getNumber() != null) query = query.equals(Exon.NUMBER_, criteria.getNumber());
			if (criteria.getPosition() != null)
			{
				// logger.debug(">>> findExons: position==" +
				...
				if (reExon.match(criteria.getPosition()))
				{
					...
					// return
					// this.findExonIdByCdna_position(Integer.valueOf(criteria.getPosition()));
				}
				else if (reIntron.match(criteria.getPosition()))
				{
					...
					if (reIntron.getParen(2).equals("+"))
					{
						...
						// logger.debug(">>> findExons: gdnaPostion==" +
						// gDNA_position + ", len==" + exon.getLength() +
						// ", pos==" + reIntron.getParen(3));
						query = query.lessOrEqual("gdna_position - length",
								gDNA_position - exon.getLength() - Integer.valueOf(reIntron.getParen(3)));
						query = query.greaterOrEqual(Exon.GDNA_POSITION,
								gDNA_position - exon.getLength() - Integer.valueOf(reIntron.getParen(3)));
						// return this.findExonIdByGdna_position(gDNA_position -
						// exon.getLength() -
						// Integer.valueOf(reIntron.getParen(3)));
					}
					else if (reIntron.getParen(2).equals("-"))
					{
						// Intron left (upstream) to this exon
						query = query.lessOrEqual("gdna_position - length",
								gDNA_position + Integer.valueOf(reIntron.getParen(3)));
						query = query.greaterOrEqual(Exon.GDNA_POSITION,
								gDNA_position + Integer.valueOf(reIntron.getParen(3)));
						// return this.findExonIdByGdna_position(gDNA_position +
						// Integer.valueOf(reIntron.getParen(3)));
			...
			if (criteria.getProteinDomainId() != null) query = query.equals(Exon.PROTEINDOMAIN,
					criteria.getProteinDomainId()).sortASC("gdna_position");
			if (criteria.getIsIntron() != null) query = query.equals(Exon.ISINTRON, criteria.getIsIntron() ? 1 : 0); // TODO:
																														// remove
																														// db
																														// specific!!!!
		...
		else if (this.db instanceof JpaDatabase)
		{
			...
			if (criteria.getCdnaPosition() != null)
			{
				...
				exonCriteria.add(cb.greaterThanOrEqualTo(exon.<Integer> get("cdna_position + length - 1"),
						criteria.getCdnaPosition()));
			}
			if (criteria.getExonId() != null) exonCriteria.add(cb.equal(exon.get("id"), criteria.getExonId()));
			if (criteria.getGdnaPosition() != null)
			{
				exonCriteria.add(cb.lessThanOrEqualTo(exon.<Integer> get("gdna_position"), criteria.getGdnaPosition()));
				exonCriteria.add(cb.greaterThanOrEqualTo(exon.<Integer> get("gdna_position + length - 1"),
						criteria.getGdnaPosition()));
			}
			if (criteria.getNumber() != null) exonCriteria.add(cb.equal(exon.get("number_"), criteria.getNumber()));
			if (criteria.getPosition() != null)
			{
				RE reExon = new RE("^(\\d+)$");
				RE reIntron = new RE("^(\\d+)([+-])(\\d+)$");
				...
				if (reExon.match(criteria.getPosition()))
				{
					exonCriteria.add(cb.lessThanOrEqualTo(exon.<Integer> get("cdna_position"),
							new Integer(criteria.getPosition())));
					exonCriteria.add(cb.greaterThanOrEqualTo(
							cb.diff(cb.sum(exon.<Integer> get("cdna_position"), exon.<Integer> get("length")), 1),
							new Integer(criteria.getPosition())));
				}
				else if (reIntron.match(criteria.getPosition()))
				{
					...
					{
						// Intron right (downstream) to this exon
						exonCriteria.add(cb.lessThanOrEqualTo(
								cb.diff(exon.<Integer> get("gdna_position"), exon.<Integer> get("length")),
								gDNA_position - tmp.getLength() - Integer.valueOf(reIntron.getParen(3))));
						exonCriteria.add(cb.greaterThanOrEqualTo(exon.<Integer> get("gdna_position"), gDNA_position
								- tmp.getLength() - Integer.valueOf(reIntron.getParen(3))));
						// query = query.lessOrEqual("gdna_position - length",
						// gDNA_position - tmp.getLength() -
						// Integer.valueOf(reIntron.getParen(3)));
						// query = query.greaterOrEqual(Exon.GDNA_POSITION,
						// gDNA_position - tmp.getLength() -
						// Integer.valueOf(reIntron.getParen(3)));
			...
			if (criteria.getProteinDomainId() != null) exonCriteria.add(cb.equal(exon.get("proteinDomain"),
					criteria.getProteinDomainId()));
			if (criteria.getIsIntron() != null) exonCriteria
					.add(cb.equal(exon.get("isIntron"), criteria.getIsIntron()));
	...
	public Exon findFirstExon(ExonSearchCriteriaVO criteria) throws DatabaseException
	{
		...
		if (criteria.getIsIntron() != null) query = query.equals(Exon.ISINTRON, criteria.getIsIntron());

		if ("F".equals(criteria.getOrientation())) query = query.sortASC(Exon.GDNA_POSITION);
		else if ("R".equals(criteria.getOrientation())) query = query.sortDESC(Exon.GDNA_POSITION);
		else
			return null;

		List<Exon> exons = query.limit(1).find();

		if (exons.size() > 0) return exons.get(0);
		else
			return null;
	}
	...
	public Exon findPrevExon(ExonSearchCriteriaVO criteria) throws DatabaseException
	{
		...
		if ("F".equals(criteria.getOrientation())) query = query.lt(Exon.GDNA_POSITION, criteria.getGdnaPosition())
				.sortDESC(Exon.GDNA_POSITION);
		else if ("R".equals(criteria.getOrientation())) query = query
				.gt(Exon.GDNA_POSITION, criteria.getGdnaPosition()).sortASC(Exon.GDNA_POSITION);
		else
			return null;
	...
	public Exon findNextExon(ExonSearchCriteriaVO criteria) throws DatabaseException
	{
		...
		if ("F".equals(criteria.getOrientation())) query = query.gt(Exon.GDNA_POSITION, criteria.getGdnaPosition())
				.sortASC(Exon.GDNA_POSITION);
		else if ("R".equals(criteria.getOrientation())) query = query
				.lt(Exon.GDNA_POSITION, criteria.getGdnaPosition()).sortDESC(Exon.GDNA_POSITION);
		else
			return null;
	...
	public Exon findLastExon(ExonSearchCriteriaVO criteria) throws DatabaseException
	{
		...
		if ("F".equals(criteria.getOrientation())) query = query.sortDESC(Exon.GDNA_POSITION);
		else if ("R".equals(criteria.getOrientation())) query = query.sortASC(Exon.GDNA_POSITION);
		else
			return null;
	...
	{
		if (this.cache.containsKey(exon.getId())) return this.cache.get(exon.getId());
	...
	private Integer findExonIdByCdna_position(Integer cdna_position) throws DatabaseException
	{
		if (this.db instanceof JDBCDatabase)
		{
			List<Tuple> result = ((JDBCDatabase) this.db)
					.sql(String
							.format("SELECT id FROM Exon WHERE isIntron = 0 AND cdna_position <= %d AND %d <= cdna_position + length - 1",
		...
		else if (this.db instanceof JpaDatabase)
		{
			javax.persistence.Query q = this.db
					...
					.createNativeQuery(
							String.format(
									"SELECT id FROM Exon WHERE isIntron = 0 AND cdna_position <= %d AND %d <= cdna_position + length - 1",
									cdna_position, cdna_position));
	...
	private Integer findExonIdByGdna_position(Integer gdna_position) throws DatabaseException
	{
		if (this.db instanceof JDBCDatabase)
		{
			List<Tuple> result = ((JDBCDatabase) this.db).sql(String.format(
					"SELECT id FROM Exon WHERE (gdna_position - length) <= %d AND %d <= gdna_position", gdna_position,
					gdna_position));
			// Should be only one result if exons and introns have been entered
			// correctly
			return result.get(0).getInt("id");
		}
		else if (this.db instanceof JpaDatabase)
		{
			javax.persistence.Query q = this.db.getEntityManager().createNativeQuery(
					String.format("SELECT id FROM Exon WHERE (gdna_position - length) <= %d AND %d <= gdna_position",
							gdna_position, gdna_position));
			return (Integer) q.getSingleResult();
		}
		else
			throw new DatabaseException("Unsupported database mapper");
	...
	public Integer findExonIdByPosition(String position) throws DatabaseException, RESyntaxException
	{
		...
		else if (reIntron.match(position))
		{
			...
			if (reIntron.getParen(2).equals("+"))
			// Intron right (downstream) to this exon
			return this.findExonIdByGdna_position(gDNA_position - exon.getLength()
					- Integer.valueOf(reIntron.getParen(3)));
			else if (reIntron.getParen(2).equals("-"))
			// Intron left (upstream) to this exon
	...
	/**
	 * Get the number of amino acids in given exon, excluding the ones only
	 * partially present
	 * 
	 * @param exon
	...
	/**
	 * Get the number of amino acids in given exon, including the ones only
	 * partially present
	 * 
	 * @param exon
	 * @return Number of amino acids
	...
	private String getNuclSequence(Exon exon) throws DatabaseException
	{
		...
		// if (!exon.getIsIntron())
		// {
		// String sequence = StringUtils.substring(gene.getSeq(), gdnaStart,
		// gdnaEnd);
		// String upSequence = sequence.toUpperCase();
		// System.out.println(">>> gdnaStart==" + gdnaStart + ", gdnaEnd==" +
		// gdnaEnd + ", seq.length==" + gene.getSeq().length());
		// System.out.println(">>> left==" +
		// StringUtils.substring(gene.getSeq(), 0, gdnaStart).length() +
		// ", right==" + StringUtils.substring(gene.getSeq(),
		// gdnaEnd).length());
		// String newSeq = StringUtils.substring(gene.getSeq(), 0, gdnaStart) +
		// upSequence + StringUtils.substring(gene.getSeq(), gdnaEnd);
		// gene.setSeq(newSeq);
		// this.db.beginTx();
		// this.db.update(gene);
		// this.db.commitTx();
