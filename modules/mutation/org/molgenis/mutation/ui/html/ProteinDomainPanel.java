...
public class ProteinDomainPanel extends HtmlInput
{
	...
	public String toHtml()
	{
		...
		result.appendln("<td align=\"center\" valign=\"bottom\" colspan=\"" + exons.size() + "\">"
				+ this.proteinDomainSummaryVO.getProteinDomain().getName() + "</td>");
		result.appendln("</tr>");
		result.appendln("<tr>");
		for (Exon exon : exons)
		{
			...
			result.appendln("<td>");
			result.appendln("<div class=\"pd" + this.proteinDomainSummaryVO.getProteinDomain().getId()
					+ "\" style=\"display: block; width: " + width + "px; height: 26px;\">");
			...
			result.appendln("<a class=\"clickable_block\" href=\"" + url + "\" alt=\"" + exon.getName() + "\" title=\""
					+ exon.getName() + "\"></a>");
			result.appendln("</div>");
			result.appendln("</td>");
		}
