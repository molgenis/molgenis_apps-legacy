...
public abstract class SearchPlugin extends IntegratedPluginController<SearchModel>
{
	...
	public Show handleRequest(Database db, Tuple request, OutputStream out)
	{
		...
		try
		{
			...
			else if (this.getModel().getAction().equals("findMutations"))
			{
				...
				if (StringUtils.isNotEmpty(request.getString("variation"))) criteria.setVariation(request
						.getString("variation"));
				if (StringUtils.isNotEmpty(request.getString("consequence"))) criteria.setConsequence(request
						.getString("consequence"));
				if (StringUtils.isNotEmpty(request.getString("mutation_id"))) criteria.setMutationId(request
						.getInt("mutation_id"));
				if (StringUtils.isNotEmpty(request.getString("mid"))) criteria.setMid(request.getString("mid"));
				if (StringUtils.isNotEmpty(request.getString("nuclno"))) criteria.setCdnaPosition(request
						.getInt("nuclno"));
				if (StringUtils.isNotEmpty(request.getString("aano"))) criteria.setCodonChangeNumber(request
						.getInt("aano"));
				if (StringUtils.isNotEmpty(request.getString("exon_id"))) criteria.setExonId(request.getInt("exon_id"));
				if (StringUtils.isNotEmpty(request.getString("exon"))) criteria.setExonName(request.getString("exon"));
				if (StringUtils.isNotEmpty(request.getString("type"))) criteria.setType(request.getString("type"));
				if (StringUtils.isNotEmpty(request.getString("domain_id"))) criteria.setProteinDomainId(request
						.getInt("domain_id"));
				if (StringUtils.isNotEmpty(request.getString("phenotype"))) criteria.setPhenotypeName(request
						.getString("phenotype"));
				if (StringUtils.isNotEmpty(request.getString("inheritance"))) criteria.setInheritance(request
						.getString("inheritance"));
				if (StringUtils.isNotEmpty(request.getString("snpbool"))) if (request.getString("snpbool").equals(
						"hide")) criteria.setReportedAsSNP(false);
				...
				((HttpServletRequestTuple) request).getRequest().setAttribute("mutationSummaryVOList",
						this.getModel().getMutationSummaryVOs());
	...
	private void handleShowMutation(Database db, Tuple request) throws DatabaseException
	{
		if (StringUtils.isNotEmpty(request.getString("mid"))) this.mutationSearchCriteriaVO.setMid(request
				.getString("mid"));
		if (StringUtils.isNotEmpty(request.getString("snpbool"))) if (request.getString("snpbool").equals("hide")) this.mutationSearchCriteriaVO
				.setReportedAsSNP(false);

		MutationService mutationService = new MutationService();
		mutationService.setDatabase(db);

		List<MutationSummaryVO> mutationSummaryVOs = mutationService.findMutations(mutationSearchCriteriaVO);
		if (mutationSummaryVOs.size() != 1) throw new DatabaseException("Unknown mutation id.");
	...
	private void handleFindPatients(Database db, Tuple request) throws DatabaseException
	{
		...
		this.getModel().setHeader(
				this.getModel().getPatientSummaryVOs().size() + " results for "
						+ this.patientSearchCriteriaVO.toString());
			...
			ServletException, IOException
	{
		...
		if (StringUtils.isNotEmpty(request.getString("term"))) this.mutationSearchCriteriaVO.setSearchTerm(request
				.getString("term"));
		...
		int numres = (this.getModel().getResult().equals("patients") ? this.getModel().getPatientSummaryVOHash()
				.keySet().size() : this.getModel().getMutationSummaryVOHash().keySet().size());
	...
	private void handleListAllMutations(Database db, Tuple request) throws DatabaseException, ParseException
	{
		...
		this.getModel().setHeader(
				this.getModel().getMutationSummaryVOs().size() + " results for \"Display all mutations\".");
	...
	private void handleShowPatient(Database db, Tuple request) throws Exception
	{
		if (StringUtils.isNotEmpty(request.getString("pid")))
		{
			...
			if (patientSummaryVOs.size() != 1) throw new Exception("Unknown patient id.");
	...
	private void handleShowProteinDomain(Database db, Tuple request) throws Exception
	{
		...
		{
			if ("R".equals(this.getModel().getGene().getOrientation())) this.reverseExons(this.getModel()
					.getProteinDomainSummaryVO());
			this.getModel().setHeader("");
		}
		if (this.getModel().getmBrowseVO().getExonList() == null)
		{
			...
			if ("R".equals(this.getModel().getGene().getOrientation())) Collections.reverse(exonList);
	...
	public void reload(Database db)
	{
		try
		{
			List<MutationGene> genes = db.query(MutationGene.class)
					.equals(MutationGene.NAME, this.getModel().getGeneName()).find();
			if (genes.size() > 0) this.getModel().setGene(genes.get(0));
			else
			...
			if (this.getModel().getmBrowseVO().getProteinDomainList() == null)
			{
				...
				// System.out.println(">>> exons before reverse: " +
				// this.genomeBrowserVO.getProteinDomainList().get(0).getAllExons().toString());
				if ("R".equals(this.getModel().getGene().getOrientation())) this.reverseExons(this.getModel()
						.getmBrowseVO().getProteinDomainList());
				// System.out.println(">>> exons after reverse: " +
				// this.genomeBrowserVO.getProteinDomainList().get(0).getAllExons().toString());
			}
	...
	private void populateShowMutationForm(Database db) throws DatabaseException, ParseException
	{
		...
		for (Mutation mutation : searchService.getAllMutations())
			mutationIdOptions.add(new ValueLabel(mutation.getIdentifier(), mutation.getCdna_Notation() + " ("
					+ mutation.getAa_Notation() + ")"));
	...
	private void populateExpertSearchForm(Database db) throws DatabaseException, ParseException, SQLException
	{
		...
		if (this.mutationSearchCriteriaVO.getVariation() != null) ((StringInput) this.getModel().getExpertSearchForm()
				.get("variation")).setValue(this.mutationSearchCriteriaVO.getVariation());

		if (this.mutationSearchCriteriaVO.getCdnaPosition() != null) ((IntInput) this.getModel().getExpertSearchForm()
				.get("nuclno")).setValue(this.mutationSearchCriteriaVO.getCdnaPosition());

		if (this.mutationSearchCriteriaVO.getCodonNumber() != null) ((IntInput) this.getModel().getExpertSearchForm()
				.get("aano")).setValue(this.mutationSearchCriteriaVO.getCodonNumber());
		...
		if (this.mutationSearchCriteriaVO.getExonId() != null) ((SelectInput) this.getModel().getExpertSearchForm()
				.get("exon_id")).setValue(this.mutationSearchCriteriaVO.getExonId());
		else
		...
		if (this.mutationSearchCriteriaVO.getType() != null) ((SelectInput) this.getModel().getExpertSearchForm()
				.get("type")).setValue(this.mutationSearchCriteriaVO.getType());
		else
		...
		if (this.mutationSearchCriteriaVO.getConsequence() != null) ((SelectInput) this.getModel()
				.getExpertSearchForm().get("consequence")).setValue(this.mutationSearchCriteriaVO.getConsequence());
		else
			((SelectInput) this.getModel().getExpertSearchForm().get("consequence")).setValue("Select consequence");

		List<MutationGene> genes = db.query(MutationGene.class)
				.equals(MutationGene.NAME, this.getModel().getGeneName()).find();
		if (genes.size() != 1) throw new DatabaseException("Not exactly one gene found for "
				+ this.getModel().getGeneName());
		...
		for (ProteinDomainSummaryVO domainVO : searchService.getAllProteinDomains())
			domainOptions
					.add(new ValueLabel(domainVO.getProteinDomain().getId(), domainVO.getProteinDomain().getName()));
		((SelectInput) this.getModel().getExpertSearchForm().get("domain_id")).setOptions(domainOptions);
		if (this.mutationSearchCriteriaVO.getProteinDomainId() != null) ((SelectInput) this.getModel()
				.getExpertSearchForm().get("domain_id")).setValue(this.mutationSearchCriteriaVO.getProteinDomainId());
		else
		...
		if (this.mutationSearchCriteriaVO.getPhenotypeId() != null) ((SelectInput) this.getModel()
				.getExpertSearchForm().get("phenotype")).setValue(this.mutationSearchCriteriaVO.getPhenotypeId());
		else
		...
		if (this.mutationSearchCriteriaVO.getInheritance() != null) ((SelectInput) this.getModel()
				.getExpertSearchForm().get("inheritance")).setValue(this.mutationSearchCriteriaVO.getInheritance());
		else
	...
	private void populateDisplayOptionsForm()
	{
		...
		if (this.mutationSearchCriteriaVO.getProteinDomainId() != null) this.getModel().getDisplayOptionsForm()
				.get("domain_id").setValue(this.mutationSearchCriteriaVO.getProteinDomainId());
		if (this.mutationSearchCriteriaVO.getExonId() != null) this.getModel().getDisplayOptionsForm().get("exon_id")
				.setValue(this.mutationSearchCriteriaVO.getExonId());
		if (this.mutationSearchCriteriaVO.getMid() != null) this.getModel().getDisplayOptionsForm().get("mid")
				.setValue(this.mutationSearchCriteriaVO.getMid());
	...
	{
		this.getModel().getmBrowseVO().getGenePanel()
				.setProteinDomainSummaryVOList(this.getModel().getmBrowseVO().getProteinDomainList());
		this.getModel()
				.getmBrowseVO()
				.getGenePanel()
				.setBaseUrl(
						"molgenis.do?__target=" + this.getName() + "&select=" + this.getName()
								+ "&__action=showProteinDomain&domain_id=&snpbool=1#exon");
	}

	private void populateProteinDomainPanel()
	{
		this.getModel().getmBrowseVO().getProteinDomainPanel()
				.setProteinDomainSummaryVO(this.getModel().getProteinDomainSummaryVO());
		this.getModel()
				.getmBrowseVO()
				.getProteinDomainPanel()
				.setBaseUrl(
						"molgenis.do?__target=" + this.getName() + "&select=" + this.getName()
								+ "&__action=showProteinDomain&domain_id=&snpbool=1#exon");
	}
	...
	private void populateExonIntronPanel()
	{
		...
		this.getModel().getmBrowseVO().getExonIntronPanel()
				.setShowIntrons(this.getModel().getQueryParametersVO().getShowIntrons());
		this.getModel()
				.getmBrowseVO()
				.getExonIntronPanel()
				.setBaseUrl(
	...
	private void populateSequencePanel()
	{
		...
		this.getModel().getmBrowseVO().getSequencePanel()
				.setMutationSummaryVOs(this.getModel().getMutationSummaryVOs());
		this.getModel()
				.getmBrowseVO()
				.getSequencePanel()
				.setBaseUrl(
