...
public abstract class Upload extends PluginModel<Entity>
{
	...
	public void handleRequest(Database db, MolgenisRequest request)
	{
		try
		{
			...
			if (StringUtils.isEmpty(this.action) || StringUtils.endsWith(this.action, "Batch")) this.handleBatch(db,
					request);
			if (StringUtils.isEmpty(this.action) || StringUtils.endsWith(this.action, "Patient")) this.handlePatient(
					db, request);
			else if (StringUtils.endsWith(this.action, "Mutation")) this.handleMutation(db, request);
		}
		catch (Exception e)
		{
	...
	private void handleBatch(Database db, Tuple request) throws Exception
	{
		...
		else if (this.action.equals("emailBatch"))
		{
			...
			String uploadPath = "http://vm7.target.rug.nl" + rt.getRequest().getContextPath() + "/res/upload/"
					+ destName;

			String emailContents = "New data upload: " + uploadPath + "\n User: " + this.getLogin().getUserName()
					+ "\n";
			// assuming: 'encoded' p.w. (setting deObf = true)
			this.getEmailService().email("New data upload for COL7A1", emailContents,
					"p.c.van.den.akker@medgen.umcg.nl", true);
			// service.email("New data upload for COL7A1", emailContents,
			// "robert.wagner42@gmail.com");
			this.getMessages().add(
					new ScreenMessage("Thank you for your submission. Your data has been successfully emailed to us.",
							true));
		}
	}
	...
	private void handlePatient(Database db, Tuple request) throws Exception
	{
		...
		else if (this.action.equals("insertPatient"))
		{
			...
			String emailContents = "New patient upload:\n" + this.patientSummaryVO.toString() + "\nUser: "
					+ this.getLogin().getUserName() + "\n";
			// assuming: 'encoded' p.w. (setting deObf = true)
			this.getEmailService().email("New patient upload for COL7A1", emailContents,
					"p.c.van.den.akker@medgen.umcg.nl", true);
			// service.email("New patient upload for COL7A1", emailContents,
			// "robert.wagner42@gmail.com");
	...
	private void handleMutation(Database db, Tuple request) throws Exception
	{
		...
		else if (this.action.equals("insertMutation"))
		{
			...
			String emailContents = "New mutation upload:\n" + this.mutationUploadVO.toString() + "\nUser: "
					+ this.getLogin().getUserName() + "\n";
			// assuming: 'encoded' p.w. (setting deObf = true)
			this.getEmailService().email("New mutation upload for COL7A1", emailContents,
					"p.c.van.den.akker@medgen.umcg.nl", true);
			// service.email("New mutation upload for COL7A1", emailContents,
			// "robert.wagner42@gmail.com");
	...
	private void populatePatientForm(Database db) throws Exception
	{
		...
		// phenotypeOptions.add(new ValueLabel(phenotype.getId(),
		// phenotype.getMajortype() + ", " + phenotype.getSubtype() + " (" +
		// phenotype.getName() + ")"));
		...
		if (this.patientSummaryVO.getVariantSummaryVOList().size() > 0) this.patientForm.get("mutation1").setValue(
				this.patientSummaryVO.getVariantSummaryVOList().get(0).getId());
		((SelectInput) this.patientForm.get("mutation2")).setOptions(mutationOptions);
		if (this.patientSummaryVO.getVariantSummaryVOList().size() > 1) this.patientForm.get("mutation2").setValue(
				this.patientSummaryVO.getVariantSummaryVOList().get(1).getId());
		this.patientForm.get("number").setValue(this.patientSummaryVO.getPatientNumber());
		// ((SelectInput)
		// this.patientForm.get("phenotype")).setOptions(phenotypeOptions);
		...
		// ((SelectInput) this.patientForm.get("blistering")).setOptions(new
		// PhenotypeDetails().getBlisteringOptions());
		// ((SelectInput) this.patientForm.get("location")).setOptions(new
		// PhenotypeDetails().getLocationOptions());
		// ((SelectInput) this.patientForm.get("hands")).setOptions(new
		// PhenotypeDetails().getHandsOptions());
		// ((SelectInput) this.patientForm.get("feet")).setOptions(new
		// PhenotypeDetails().getFeetOptions());
		// ((SelectInput) this.patientForm.get("arms")).setOptions(new
		// PhenotypeDetails().getArmsOptions());
		// ((SelectInput) this.patientForm.get("legs")).setOptions(new
		// PhenotypeDetails().getLegsOptions());
		// ((SelectInput)
		// this.patientForm.get("proximal_body_flexures")).setOptions(new
		// PhenotypeDetails().getProximal_Body_FlexuresOptions());
		// ((SelectInput) this.patientForm.get("trunk")).setOptions(new
		// PhenotypeDetails().getTrunkOptions());
		// ((SelectInput)
		// this.patientForm.get("mucous_membranes")).setOptions(new
		// PhenotypeDetails().getMucous_MembranesOptions());
		// ((SelectInput) this.patientForm.get("skin_atrophy")).setOptions(new
		// PhenotypeDetails().getSkin_AtrophyOptions());
		// ((SelectInput) this.patientForm.get("milia")).setOptions(new
		// PhenotypeDetails().getMiliaOptions());
		// ((SelectInput) this.patientForm.get("nail_dystrophy")).setOptions(new
		// PhenotypeDetails().getNail_DystrophyOptions());
		// ((SelectInput)
		// this.patientForm.get("albopapuloid_papules")).setOptions(new
		// PhenotypeDetails().getAlbopapuloid_PapulesOptions());
		// ((SelectInput)
		// this.patientForm.get("pruritic_papules")).setOptions(new
		// PhenotypeDetails().getPruritic_PapulesOptions());
		// ((SelectInput) this.patientForm.get("alopecia")).setOptions(new
		// PhenotypeDetails().getAlopeciaOptions());
		// ((SelectInput)
		// this.patientForm.get("squamous_cell_carcinomas")).setOptions(new
		// PhenotypeDetails().getSquamous_Cell_CarcinomasOptions());
		// ((SelectInput)
		// this.patientForm.get("revertant_skin_patch")).setOptions(new
		// PhenotypeDetails().getRevertant_Skin_PatchOptions());
		// ((SelectInput)
		// this.patientForm.get("flexion_contractures")).setOptions(new
		// PhenotypeDetails().getFlexion_ContracturesOptions());
		// ((SelectInput)
		// this.patientForm.get("pseudosyndactyly_hands")).setOptions(new
		// PhenotypeDetails().getPseudosyndactyly_HandsOptions());
		// ((SelectInput) this.patientForm.get("microstomia")).setOptions(new
		// PhenotypeDetails().getMicrostomiaOptions());
		// ((SelectInput) this.patientForm.get("ankyloglossia")).setOptions(new
		// PhenotypeDetails().getAnkyloglossiaOptions());
		// ((SelectInput) this.patientForm.get("dysphagia")).setOptions(new
		// PhenotypeDetails().getDysphagiaOptions());
		// ((SelectInput)
		// this.patientForm.get("growth_retardation")).setOptions(new
		// PhenotypeDetails().getGrowth_RetardationOptions());
		// ((SelectInput) this.patientForm.get("anemia")).setOptions(new
		// PhenotypeDetails().getAnemiaOptions());
		// ((SelectInput) this.patientForm.get("renal_failure")).setOptions(new
		// PhenotypeDetails().getRenal_FailureOptions());
		// ((SelectInput)
		// this.patientForm.get("dilated_cardiomyopathy")).setOptions(new
		// PhenotypeDetails().getDilated_CardiomyopathyOptions());

		// IF
		// ((SelectInput) this.patientForm.get("if_value")).setOptions(new
		// I_F().getValueOptions());
		// ((SelectInput) this.patientForm.get("if_retention")).setOptions(new
		// I_F().getRetentionOptions());

		// EM
		// ((SelectInput) this.patientForm.get("em_fibrils")).setOptions(new
		// E_M().getNumberOptions());
		// ((SelectInput) this.patientForm.get("em_appearance")).setOptions(new
		// E_M().getAppearanceOptions());
		// ((SelectInput) this.patientForm.get("em_retention")).setOptions(new
		// E_M().getRetentionOptions());
	}
	...
	private void populateMutationForm(Database db) throws DatabaseException, ParseException
	{
		...
		if (this.mutationUploadVO.getMutation().getConservedAA() != null) if (this.mutationUploadVO.getMutation()
				.getConservedAA()) conservedValue.add("conservedaa");
		else
			conservedValue.add("");

		Vector<String> founderValue = new Vector<String>();
		if (this.mutationUploadVO.getMutation().getFounderMutation() != null) if (this.mutationUploadVO.getMutation()
				.getFounderMutation()) founderValue.add("foundermutation");
		else
			founderValue.add("");

		Vector<String> snpValue = new Vector<String>();
		if (this.mutationUploadVO.getMutation().getReportedSNP() != null) if (this.mutationUploadVO.getMutation()
				.getReportedSNP()) snpValue.add("reportedsnp");
		else
			snpValue.add("");
		...
		((SelectInput) this.mutationForm.get("event")).setOnchange("toggleForm(this.value);"); // TODO:
																								// Unhack
																								// this
		...
		if (this.mutationUploadVO.getExonId() != null) this.mutationForm.get("exon").setValue(
				this.mutationUploadVO.getExonId());
		...
		// ((SelectInput) this.mutationForm.get("type")).setOptions(new
		// Mutation().getTypeOptions());
	...
	private PatientSummaryVO toPatientSummaryVO(Tuple request)
	{
		...
		if (StringUtils.isNotEmpty(request.getString("number"))) patientSummaryVO.setPatientNumber(request
				.getString("number"));
		...
		if (StringUtils.isNotEmpty(request.getString("age"))) patientSummaryVO.setPatientAge(request.getString("age"));

		if (StringUtils.isNotEmpty(request.getString("gender"))) patientSummaryVO.setPatientGender(request
				.getString("gender"));

		if (StringUtils.isNotEmpty(request.getString("ethnicity"))) patientSummaryVO.setPatientEthnicity(request
				.getString("ethnicity"));

		if (StringUtils.isNotEmpty(request.getString("deceased"))) patientSummaryVO.setPatientDeceased(request
				.getString("deceased"));

		if (StringUtils.isNotEmpty(request.getString("deatch_cause"))) patientSummaryVO.setPatientDeathCause(request
				.getString("death_cause"));

		if (StringUtils.isNotEmpty(request.getString("mmp1_allele1"))) patientSummaryVO.setPatientMmp1Allele1(request
				.getString("mmp1_allele1"));

		if (StringUtils.isNotEmpty(request.getString("mmp1_allele2"))) patientSummaryVO.setPatientMmp1Allele2(request
				.getString("mmp1_allele2"));

		if (StringUtils.isNotEmpty(request.getString("consent"))) patientSummaryVO.setPatientConsent(request
				.getString("consent"));
	...
	//
	// phenotypeDetailsVO.setObservedValues(new HashMap<String,
	// List<ObservedValueVO>>());
	//
	...
	// // if
	// (StringUtils.isNotEmpty(request.getString("proximal_body_flexures")))
	// //
	...
	// // if
	// (StringUtils.isNotEmpty(request.getString("squamous_cell_carcinomas")))
	// //
	...
	// // if
	// (StringUtils.isNotEmpty(request.getString("pseudosyndactyly_hands")))
	// //
	...
	// // if
	// (StringUtils.isNotEmpty(request.getString("dilated_cardiomyopathy")))
	// //
	...
	{
		if (StringUtils.isNotEmpty(request.getString("referer"))) this.referer = request.getInt("referer");

		if (StringUtils.isNotEmpty(request.getString("consequence"))) this.mutationUploadVO.getMutation()
				.setConsequence(request.getString("consequence"));

		if (StringUtils.isNotEmpty(request.getString("inheritance"))) this.mutationUploadVO.getMutation()
				.setInheritance(request.getString("inheritance"));

		if (StringUtils.isNotEmpty(request.getString("conservedaa"))) if (StringUtils.equals(
				request.getString("conservedaa"), "conservedaa")) this.mutationUploadVO.getMutation().setConservedAA(
				true);
		else
			this.mutationUploadVO.getMutation().setConservedAA(false);

		if (StringUtils.isNotEmpty(request.getString("effectonsplicing"))) if (StringUtils.equals(
				request.getString("effectonsplicing"), "effectonsplicing")) this.mutationUploadVO.getMutation()
				.setEffectOnSplicing(true);
		else
			this.mutationUploadVO.getMutation().setEffectOnSplicing(false);

		if (StringUtils.isNotEmpty(request.getString("event"))) this.mutationUploadVO.getMutation().setEvent(
				request.getString("event"));

		if (StringUtils.isNotEmpty(request.getString("foundermutation"))) if (StringUtils.equals(
				request.getString("foundermutation"), "foundermutation")) this.mutationUploadVO.getMutation()
				.setFounderMutation(true);
		else
		...
		if (StringUtils.isNotEmpty(request.getString("length"))) this.mutationUploadVO.getMutation().setLength(
				request.getInt("length"));

		if (StringUtils.isNotEmpty(request.getString("ntchange"))) this.mutationUploadVO.getMutation().setNtchange(
				request.getString("ntchange").toUpperCase());

		if (StringUtils.isNotEmpty(request.getString("population"))) this.mutationUploadVO.getMutation().setPopulation(
				request.getString("population"));

		if (StringUtils.isNotEmpty(request.getString("position"))) this.mutationUploadVO.getMutation()
				.setMutationPosition(request.getString("position"));
		else
			this.mutationUploadVO.getMutation().setMutationPosition("");
		// throw new Exception("Please enter the position.");

		if (StringUtils.isNotEmpty(request.getString("reportedsnp"))) if (StringUtils.equals(
				request.getString("reportedsnp"), "reportedsnp")) this.mutationUploadVO.getMutation().setReportedSNP(
				true);
		else
			;
		else
		...
		if (StringUtils.isNotEmpty(request.getString("aachange"))) this.mutationUploadVO.setAachange(request
				.getString("aachange"));

		if (StringUtils.isNotEmpty(request.getString("type"))) this.mutationUploadVO.getMutation().setType(
	...
	public void reload(Database db)
	{
		try
		{
			...
			{
				this.initMutationUploadVO(db);
				this.mutationUploadVO.setGeneSymbol(db.query(MutationGene.class)
						.equals(MutationGene.NAME, this.GENENAME).find().get(0).getSymbol());
			}

			if (patientSummaryVO == null) this.initPatientSummaryVO();
		}
		catch (Exception e)
		{
