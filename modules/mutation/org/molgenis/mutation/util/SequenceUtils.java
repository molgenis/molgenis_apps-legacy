...
public class SequenceUtils
{
	...
	{
		GCT("A", "Ala"), GCC("A", "Ala"), GCA("A", "Ala"), GCG("A", "Ala"), CGT("R", "Arg"), CGC("R", "Arg"), CGA("R",
				"Arg"), CGG("R", "Arg"), AGA("R", "Arg"), AGG("R", "Arg"), AAT("N", "Asn"), AAC("N", "Asn"), GAT("D",
				"Asp"), GAC("D", "Asp"), TGT("C", "Cys"), TGC("C", "Cys"), CAA("Q", "Gln"), CAG("Q", "Gln"), GAA("E",
				"Glu"), GAG("E", "Glu"), GGT("G", "Gly"), GGC("G", "Gly"), GGA("G", "Gly"), GGG("G", "Gly"), CAT("H",
				"His"), CAC("H", "His"), ATT("I", "Ile"), ATC("I", "Ile"), ATA("I", "Ile"), TTA("L", "Leu"), TTG("L",
				"Leu"), CTT("L", "Leu"), CTC("L", "Leu"), CTA("L", "Leu"), CTG("L", "Leu"), AAA("K", "Lys"), AAG("K",
				"Lys"), ATG("M", "Met"), TTT("F", "Phe"), TTC("F", "Phe"), CCT("P", "Pro"), CCC("P", "Pro"), CCA("P",
				"Pro"), CCG("P", "Pro"), TCT("S", "Ser"), TCC("S", "Ser"), TCA("S", "Ser"), TCG("S", "Ser"), AGT("S",
				"Ser"), AGC("S", "Ser"), TAA("X", "Ter"), TAG("X", "Ter"), TGA("X", "Ter"), ACT("T", "Thr"), ACC("T",
				"Thr"), ACA("T", "Thr"), ACG("T", "Thr"), TGG("W", "Trp"), TAT("Y", "Tyr"), TAC("Y", "Tyr"), GTT("V",
				"Val"), GTC("V", "Val"), GTA("V", "Val"), GTG("V", "Val");
	...
	public static String getAminoAcid1(String codon)
	{
		for (AminoAcid aminoAcid : AminoAcid.values())
			if (aminoAcid.name().equals(codon)) return aminoAcid.getCode_1();
		return "?";
	}
	...
	{
		for (AminoAcid aminoAcid : AminoAcid.values())
			if (aminoAcid.name().equals(codon)) return aminoAcid.getCode_3();
		return "???";
	}
	...
	{
		for (AminoAcid aminoAcid : AminoAcid.values())
			if (aminoAcid.getCode_1().equals(code1)) return aminoAcid.getCode_3();
		return "???";
	}

	/**
	 * Get the number of amino acids in given sequence, excluding the ones only
	 * partially present
	 * 
	 * @param sequence
	...
	public static int getNumFullAminoAcids(String sequence)
	{
		...
		if (tokens[numFullAas - 1].length() < 3) numFullAas--;

		return numFullAas;
	}

	/**
	 * Get the number of amino acids in given sequence, including the ones only
	 * partially present
	 * 
	 * @param sequence
	 * @return Number of amino acids
	...
	public static Integer getNumGlyXYRepeats(String sequence)
	{
		...
		for (int i = 0; i < sequence.length(); i++)
		{
			...
			while (glyPos + 8 < sequence.length())
			{
				localNumGly++;
				glyPos = glyPos + 9;
				if (!StringUtils.substring(sequence, glyPos, glyPos + 3).equals("Gly")) break;
			}
			if (localNumGly > numGly) numGly = localNumGly;
		}
	...
	/**
	 * Splice sequence, i.e. remove intronic part which has to be in lowercase
	 * letters
	 * 
	 * @param sequence
	 ...
	 * @param sequence
	 * @param startCodonNum
	 * @return codon number of first occurence, -1 if no match or null sequence
	 *         input
	 */
	...
	public static String getCodon(String sequence, int codonNum)
	{
		...
		/*
		 * for (int i = 0; i < sequence.length(); i = i + 3) if ((i + 3) / 3 ==
		 * codonNum) return sequence.substring(i, i + 3); return "";
		 */
	}
	...
	/**
	 * Get the cDNA position. In case of an intronic position remove additive
	 * part (e.g. 1234+5 -> 1234)
	 * 
	 * @param position
	...
	public static Integer getCDNAPosition(String position) throws RESyntaxException
	{
		...
		if (reExon.match(position)) return new Integer(reExon.getParen(1));
		else if (reIntron.match(position)) return new Integer(reIntron.getParen(1));
		else
	...
	public static String getAddedPosition(String position, Integer add) throws RESyntaxException
	{
		...
		if (reExon.match(position)) return new Integer(new Integer(reExon.getParen(1)) + add).toString();
		else if (reIntron.match(position)) if (reIntron.getParen(2).equals("-")) if (add < new Integer(
				reIntron.getParen(3))) return reIntron.getParen(1) + reIntron.getParen(2)
				+ (new Integer(reIntron.getParen(3)).intValue() - add.intValue());
		else
			return String.valueOf(new Integer(reIntron.getParen(1)).intValue()
					- new Integer(reIntron.getParen(3)).intValue() + add.intValue());
		else
			return reIntron.getParen(1) + reIntron.getParen(2)
					+ (new Integer(reIntron.getParen(3)).intValue() + add.intValue());
		else
			throw new RESyntaxException("Invalid position notation: " + position);
	}
	 ...
	 * @throws RESyntaxException
	 */
	public static Integer getGDNAPosition(String position, Exon exon, String orientation) throws RESyntaxException // Integer
																													// gDNAPositionStart,
																													// Integer
																													// cDNAPositionStart)
	{
		...
		if (reExon.match(position)) if ("R".equals(orientation))
		...
		return Math.abs(exon.getGdna_Position() - (Integer.valueOf(reExon.getParen(1)) - exon.getCdna_Position()));
			...
			// exon.gDNA + (mutation.cDNA - exon.cDNA)
			return Math.abs(exon.getGdna_Position() + (Integer.valueOf(reExon.getParen(1)) - exon.getCdna_Position()));
		else if (reIntron.match(position)) if (reIntron.getParen(2).equals("+")) if ("R".equals(orientation))
		...
		return Math.abs(exon.getGdna_Position() + 1 - Integer.valueOf(reIntron.getParen(3)));
			...
			// intron.gdnaPos - 1 (start at last position of exon) + difference
			// ('+' means upstream)
			return Math.abs(exon.getGdna_Position() - 1 + Integer.valueOf(reIntron.getParen(3)));
		else if ("R".equals(orientation))
