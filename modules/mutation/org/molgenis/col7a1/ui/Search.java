...
public class Search extends SearchPlugin
{
	...
	public void reload(Database db)
	{
		...
		try
		{
			...
			text.appendln("</p>");
			text.appendln("<p>");
			text.appendln("The database currently contains " + this.getModel().getNumPatients()
					+ " DEB patients, of which " + this.getModel().getNumUnpublished() + " unpublished, and "
					+ this.getModel().getNumMutations() + " COL7A1 mutations. Search or browse below.");
			text.appendln("</p>");
			text.appendln("<br/>");
			...
			// fee =
			// db.query(FrontEndElement.class).equals(FrontEndElement.TYPE_,
			// "search").find();
			//
			// if (fee.size() == 1)
			// {
