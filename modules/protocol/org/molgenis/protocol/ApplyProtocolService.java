...
/**
 * A subset of the AnimalDB-specific CommonService functionality. These methods
 * were copied so the protocol module can function independently from AnimalDB.
 * TODO: get rid of the resulting code duplication. Maybe factor out
 * CommonService altogether?
 * 
 * @author erikroos
 * 
 */
public class ApplyProtocolService
{
	...
	public List<ObservedValue> getObservedValuesByTargetAndFeature(Database db, int targetId, Measurement measurement,
	...
	public List<ObservedValue> getObservedValuesByTargetAndFeatures(Database db, int targetId,
			List<Measurement> measurements, List<Integer> investigationIds, int investigationToAddToId)
	...
	public List<Measurement> getMeasurementsByProtocol(Database db, String protocolName) throws DatabaseException,
			ParseException
	{
	...
	public List<String> getAllCodesForFeatureAsStrings(Database db, String featurename) throws DatabaseException,
			ParseException
	{
	...
	public List<ObservationTarget> getObservationTargets(Database db, List<Integer> idList) throws DatabaseException,
			ParseException
	{
		if (idList.size() > 0)
		{
			...
			throws DatabaseException, ParseException
	{
		...
		valueQuery.addRules(new QueryRule(qr1, qr2, qr3)); // only user's own OR
															// System
															// investigations
	...
	public List<Protocol> getAllProtocolsSorted(Database db, String sortField, String sortOrder,
	...
	public ObservationTarget getObservationTargetById(Database db, int targetId) throws DatabaseException,
			ParseException
	{
