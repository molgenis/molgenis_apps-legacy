...
public class ApplyProtocolUI
{
	 ...
	 * 
	 * @param col
	 *            column to insert the input in
	 * @param row
	 *            row to insert the input in
	 * @param order
	 *            in case of multiple HtmlInputs for one target-feature
	 *            combination, append this number to keep the input name unique
	 * @param value
	 *            the value to insert
	 * @throws Exception
	 */
	@SuppressWarnings(
	{ "rawtypes", "unchecked" })
	private HtmlInput makeInput(Database db, int featureNr, int col, int row, int order, ObservedValue value)
			throws Exception
	{
		...
		{
			// If there are categories for this Measurement, show a selectbox
			// with those
		...
		else
		{
			if (panelLabel != null)
			{
				// If there's only a subset of labeled Panels allowed for this
				// Measurement, show a selectbox with those
				valueInput = new SelectInput(col + "_" + row + "_" + order);
	...
	/**
	 * Makes the appropriate input for the cell at col, row and adds it to the
	 * valueTable. At the moment, supports only cells which contain one input.
	 * 
	 * @param featureNr
	 * @param col
	 * @param row
	...
	public void makeInputAndSetCell(Database db, int featureNr, int col, int row, int order, ObservedValue value)
			throws Exception
	{
	...
	/**
	 * Makes the appropriate date input for the cell at col, row and adds it to
	 * the valueTable. At the moment, supports only cells which contain one
	 * input.
	 * 
	 * @param col
	 * @param row
	...
	// service.getWritableUserInvestigationIds(model.getUserId());
	// for (ObservationTarget o :
	// service.getAllObservationTargets(investigationIds)) {
	// targets.addOption(o.getId(),
	// service.getObservationTargetById(o.getId()).getName());
	// }
	// protocolDiv.add(targets);
	//
	// } catch(Exception e) {
	// e.printStackTrace();
	// logger.error("An error occurred while retrieving targets from the database",
	// e);
	// }
	// }
	...
	private void makeNewOrEditButtons(String userName) throws HtmlInputException
	{
		...
		newOrEditButtons = new RadioInput("NewOrEdit", "", "New", false, false,
				"Indicate whether you want to fill in new values (default) or edit existing ones.", options,
				optionLabels);
	...
	/**
	 * Create a checkbox to toggle showing all values, not only the most recent
	 * one(s).
	 */
	private void makeAllValuesSelectbox()
	{
		...
		allValuesBox = new CheckboxInput("AllValuesBox", "",
				"Indicate whether you want to see all values for every target-measurement combination", options, null);
		protocolDiv.add(new Paragraph("", "")); // gives empty <p></p>
	...
	/**
	 * Create columns for a given protocol id, with the columns being the
	 * features for that protocol
	 * 
	 * @param protocolId
	 *            the protocol whose features we want to use
	...
	public void fillTableCells(Database db) throws DatabaseException, ParseException
	{
		try
		{
			...
			// Rest of the rows contain inputs for each target-feature
			// combination
			for (int row = 1; row <= model.getFullTargetList().size(); row++)
			{
				...
				for (int col = 0; col < sizeFeatures; col++)
				{
					...
					{
						// Show existing values, or new ones if none can be
						// found
						DivPanel valueDiv = new DivPanel();
						DivPanel starttimeDiv = new DivPanel();
						DivPanel endtimeDiv = new DivPanel();
						int valueCounter = 0;
						List<ObservedValue> values = service.getObservedValuesByTargetAndFeature(db, model
								.getTargetsIdList().get(row - 1), model.getFeaturesList().get(col), investigationIds,
								ownInvId);
						for (ObservedValue value : values)
						{
							...
							if (model.isTimeInfo())
							{
								...
								DatetimeInput datetimeInputEnd = new DatetimeInput((colNrInTable + 2) + "_" + row + "_"
										+ valueCounter);
								datetimeInputEnd.setLabel("");
	...
	public void setValues()
	{
		...
		if (model.isTimeInfo())
		{
			...
			timeBox.setValue(valueVector); // checkboxInput's setValue() expects
											// a vector of String (undocumented
											// behavior)
		}
		else
		{
		...
		if (model.isAllValues())
		{
			...
			allValuesBox.setValue(valueVector); // checkboxInput's setValue()
												// expects a vector of String
	...
	public void fixCellValue(Database db, int col, int row, Object value)
	{
		...
		// And, if this involves an xref box, set the value and label of the
		// selected option
