...
public class ComputeExecutorPilotDB implements ComputeExecutor
{
	...
	public void executeTasks(String backend, String backendType)
	{
		...
		try
		{
			...
			// List<ComputeTask> generatedTasks = db.find(ComputeTask.class, new
			// QueryRule(ComputeTask.STATUSCODE, QueryRule.Operator.EQUALS,
			// "generated"));

			List<ComputeTask> generatedTasks = db.query(ComputeTask.class).equals(ComputeTask.STATUSCODE, "generated")
			...
			// List<ComputeTask> readyTasks = db.find(ComputeTask.class, new
			// QueryRule(ComputeTask.STATUSCODE, QueryRule.Operator.EQUALS,
			// "ready"));
			List<ComputeTask> readyTasks = db.query(ComputeTask.class).equals(ComputeTask.STATUSCODE, "ready")
					.equals(ComputeTask.BACKENDNAME, backend).find();
		...
		for (int i = 0; i < readyToSubmitSize; i++)
		{
			try
			{
				if (backendType.equalsIgnoreCase(BACK_END_GRID)) host.submitPilotGrid();
				else if (backendType.equalsIgnoreCase(BACK_END_CLUSTER)) host.submitPilotCluster();
				else if (backendType.equalsIgnoreCase(BACK_END_LOCALHOST)) submitPilotLocalhost();
			}
			catch (IOException e)
			{
				e.printStackTrace();
	...
	private int evaluateTasks(List<ComputeTask> generatedTasks) throws DatabaseException
	{
		...
		for (ComputeTask task : generatedTasks)
		{
			...
			{
				// System.out.println("prevtask " + prev.getName() + " -> " +
				// prev.getStatusCode());
