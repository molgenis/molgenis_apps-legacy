...
public class WorkflowImporterPilotDemo
{
	...
	public static void main(String[] args)
	{
		...
		try
		{
			new WorkflowImporterPilotDemo().process(args[0], args[1], args[2], args[3]);
		}
			...
			if (protocolsDir.isDirectory())
			{
				...
				if (files.length > 0)
				{
					for (int i = 0; i < files.length; i++)
					{
						...
						// TODO here we need to agree on flags and reuse code
						// from compute3 or compute4
						// also why do we have requirements here if we have xref
						// to ComputeRequirements
