...
# Files
refGenFile			= 'simulatedReferenceGenome.fa'
leftReadsFile		= 'simulatedLeft.fq'
rightReadsFile		= 'simulatedRight.fq'
visualizationFile	= 'visualization.txt'
bedFileScript		= 'simulate_bedFiles.bash.sh'
targetBedFile		= 'simulated_targets.bed'
baitBedFile			= 'simulated_baits.bed'
...
file.remove(refGenFile)
for (i.chrom in chromosomes)
{
	cat('>', i.chrom, '\n', file = refGenFile, append = T)
	for (i.line in 1:n.lines.per.chrom)
	{
		index.start = (i.line - 1) * line.length
		cat(refGen[[i.chrom]][(index.start + 1) : (index.start + line.length)], '\n', sep='', file = refGenFile, append = T)
	}
}

# Save reads
file.remove(leftReadsFile)
file.remove(rightReadsFile)
for (i in 1:n.reads)
{
	# left
	read.id = paste(read.chrom[i], '_read_', i, '_0/1\n', sep='')
	cat('@', read.id, sep='', file = leftReadsFile, append = T)
	cat(reads.left[[i]], '\n', sep='', file = leftReadsFile, append = T)
	cat('+', read.id, sep='', file = leftReadsFile, append = T)
	cat(rep(2, read.length[i]), '\n', sep='', file = leftReadsFile, append = T)

	# right
	read.id = paste(read.chrom[i], '_read_', i, '_0/2\n', sep='')
	cat('@', read.id, sep='', file = rightReadsFile, append = T)
	cat(reads.left[[i]], '\n', sep='', file = rightReadsFile, append = T)
	cat('+', read.id, sep='', file = rightReadsFile, append = T)
	cat(rep(2, read.length[i]), '\n', sep='', file = rightReadsFile, append = T)
}

# Visualize Reference Genome and reads
file.remove(visualizationFile)
for (i.chrom in chromosomes)
{
	cat('> Chromosome', i.chrom, '\n', file = visualizationFile, append = T)
	if (SNP.chrom == i.chrom) cat(rep(' ', min(SNP.location) - 1), '| SNP\n', sep='', file = visualizationFile, append = T)
...
# Generate script to create bed file
file.remove(bedFileScript)
cat("rm -f", targetBedFile, "\n", file = bedFileScript, append = T)
cat("rm -f", baitBedFile, "\n", file = bedFileScript, append = T)
for (i.chrom in chromosomes)
{
	cat("echo -e \"", i.chrom, "\\t1\\t", n.lines.per.chrom * line.length, "\" >> ", targetBedFile , "\n", sep='', file = bedFileScript, append = T)
	cat("echo -e \"", i.chrom, "\\t1\\t", n.lines.per.chrom * line.length, "\" >> ", baitBedFile , "\n", sep='', file = bedFileScript, append = T)
}

cat("After finishing this simulation, you may want to execute the following scripts:\n")
cat("sh createIndex.sh\n")
cat("sh createDictionary.sh\n")
cat("bash simulate_bedFiles.bash.sh\n")
cat("sh create_interval_list.sh\n")
