...
java -jar $PICARD_HOME/CollectGcBiasMetrics.jar \
R=${indexfile} \
I=${sortedbam} \
O=${gcbiasmetrics} \
...
VALIDATION_STRINGENCY=LENIENT \
TMP_DIR=${tempdir} \
WINDOW_SIZE=2 # This option was added because for our small simulated data set with default settings this jar produces errors and no sensible output!

<#if seqType == "PE">
	java -jar $PICARD_HOME/CollectInsertSizeMetrics.jar \
	I=${sortedbam} \
	O=${insertsizemetrics} \
...
java -jar $PICARD_HOME/MeanQualityByCycle.jar \
I=${sortedbam} \
...
java -jar $PICARD_HOME/QualityScoreDistribution.jar \
I=${sortedbam} \
...
java -jar $PICARD_HOME/BamIndexStats.jar \
INPUT=${sortedbam} \
VALIDATION_STRINGENCY=LENIENT \
TMP_DIR=${tempdir} \
> ${bamindexstats}
