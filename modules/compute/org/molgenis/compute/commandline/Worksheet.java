...
public class Worksheet
{
	...
	/*
	 * private void fillConstants() { boolean foundMore = true; while
	 * (foundMore) { foundMore = false; for (ComputeParameter cp :
	 * parameterlist) { // first set the hasOne if (cp.getDefaultValue() != null
	 * && cp.getDefaultValue().contains("${")) { for (ComputeParameter otherCp :
	 * parameterlist) { // for constants we know: if
	 * (cp.getDefaultValue().contains("${" + otherCp.getName() + "}") &&
	 * otherCp.getDefaultValue() != null &&
	 * !otherCp.getDefaultValue().contains("${")) { // for constants, we know it
	 * works if (!cp.getHasOne_Name().contains(otherCp.getName())) {
	 * cp.setDefaultValue(cp.getDefaultValue().replace("${" + otherCp.getName()
	 * + "}", otherCp.getDefaultValue())); }
	 * 
	 * } } } } }
	 * 
	 * // pretty print // for (ComputeParameter cp : parameterlist) // { //
	 * System.out.println(cp.getName() +
	 * " defaultValue: '"+cp.getDefaultValue()+"', hasone: " +
	 * cp.getHasOne_Name()); // } // throw new RuntimeException("BREAK"); }
	 */
