...
public class StartCommandLineGeneration
{
	...
	private void run() throws Exception
	{
		...
		generator.setLocalLocation(path + System.getProperty("file.separator") + applicationName
				+ System.getProperty("file.separator"));

		// here the loop over samples/lanes
		generator.setNewRun();

		generator.processSingleWorksheet(computeBundle, userValues, ""/* + iii */, this.backend, templateDir);
		// add generated applications to the bundle
		computeBundle.setComputeJobs(generator.getComputeApplications());
		// iii++;
		// }

		if (backend.equalsIgnoreCase(WorkflowGeneratorCommandLine.CLUSTER)) generator.flashSumbitScript();
	}
	...
	// lazy code
	private static void checkSynonymParameters(StartCommandLineGeneration generation, Options opt, String backend,
			String backend1, String backend2, String backend3, String backend4)
	{
		if (!(opt.getSet().isSet(backend1) ^ opt.getSet().isSet(backend2) ^ opt.getSet().isSet(backend3) ^ opt.getSet()
				.isSet(backend4)))
		{
	...
	private static void checkSynonymParameters(StartCommandLineGeneration generation, Options opt, String parameter,
			String str1, String str2)
	{
		...
		if (opt.getSet().isSet(str1)) result = opt.getSet().getOption(str1).getResultValue(0);
		else if (opt.getSet().isSet(str2)) result = opt.getSet().getOption(str2).getResultValue(0);
