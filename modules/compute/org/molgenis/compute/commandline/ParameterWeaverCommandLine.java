...
/**
 * Created by IntelliJ IDEA. User: georgebyelas Date: Mar 14, 2011 Time: 1:31:21
...
public class ParameterWeaverCommandLine
{
	...
	private String verificationTemplate = "java -jar /data/gcc/tools/GATK-1.0.5069/Sting/dist/GenomeAnalysisTK.jar "
			...
			+ "-I /data/gcc/test_george/819/110214_SN163_391A80MTLABXX_4_AGAGAT.sorted.bam " + "-T CountReads "
	...
	private void readTemplates(String templateDir)
	{
		try
		{
			scriptClusterTemplate = getFileAsString(templateDir + System.getProperty("file.separator")
					+ "templ-cluster.ftl");
			submitTemplate = getFileAsString(templateDir + System.getProperty("file.separator") + "templ-submit.ftl");
			downloadGridTemplate = getFileAsString(templateDir + System.getProperty("file.separator")
					+ "templ-download-grid.ftl");
			executableGridTemplate = getFileAsString(templateDir + System.getProperty("file.separator")
					+ "templ-exe-grid.ftl");
			uploadGridTemplate = getFileAsString(templateDir + System.getProperty("file.separator")
					+ "templ-upload-grid.ftl");
			...
			executionGridDirectory = getFileAsString(templateDir + System.getProperty("file.separator")
					+ "templ-exe-grid-dir.ftl");
	...
	{
		return gridHeader; // To change body of created methods use File |
	...
	public String processGridHeader(String element, String template, Hashtable<String, String> weavingValues)
	{
		...
		for (int i = 0; i < inputs.size(); i++)
		{
			...
			// here generate header (downloading part for the grid + replace
			// weaving value is a new one for the grid)
			...
			String result = "";
			if (element.equalsIgnoreCase(DO_DOWNLOAD)) result = weaveFreemarker(downloadGridTemplate, local);
			else if (element.equalsIgnoreCase(DO_EXECUTABLE)) result = weaveFreemarker(executableGridTemplate, local);
			else if (element.equalsIgnoreCase(DO_UPLOAD)) result = weaveFreemarker(uploadGridTemplate, local);
			header += result;
		}
