 ...
 * user
 * <li>StartNgsModel holds application state and business logic on top of domain
 * model. Get it via this.getModel()/setModel(..)
 * <li>StartNgsView holds the template to show the layout. Get/set it via
...
public class StartNgs extends EasyPluginController<StartNgsView>
{
	...
	public void buttonStart(Database db, Tuple request) throws Exception
	{
		...
		// if(true) throw new
		// RuntimeException("I have edited line 79 of StartNgs, okay?");
		...
		for (UserParameter userParameter : userParameterList)
		{
			...
			userValues.put(featureName, "" + data.get(databaseField));// casting
																		// to
																		// String
																		// does
																		// not
		...
		applicationName = data.get("lane") + "_" + data.get("flowcell") + "_" + data.get("externalSampleID") + "_"
				+ processing.getFormattedTime();
