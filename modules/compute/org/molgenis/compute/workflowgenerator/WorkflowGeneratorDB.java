...
public class WorkflowGeneratorDB
{
			...
			throws Exception
	{
		...
		allComputeParameters = db.query(ComputeParameter.class).equals(ComputeParameter.WORKFLOW, workflow.getId())
				.find();
		...
		List<WorkflowElement> workflowElements = db.query(WorkflowElement.class)
				.equals(WorkflowElement.WORKFLOW, workflow.getId()).find();
	...
	private void createDatabaseUpdater(MCF mcf)
	{
		...
		{
			if (mcf.getBasis().equalsIgnoreCase(MCF.GRID)) updater = new DatabaseUpdaterGridGain(mcf);
			else if ((mcf.getBasis().equalsIgnoreCase(MCF.SSH))) updater = new DatabaseUpdaterSsh(mcf);
		}
	...
	public void executePipeline(Database db, Pipeline pipeline)
	{
		...
		{
			if (env.equalsIgnoreCase(ENV_CLUSTER)) mcf.setClusterPipeline(pipeline);
			else if (env.equalsIgnoreCase(ENV_GRID)) mcf.setGridPipeline(pipeline);
			...
			throws DatabaseException, ParseException, IOException
	{
		...
		{
			if (computeFeature.getIsUser()) continue;
			...
			throws IOException, DatabaseException, ParseException
	{
		...
		if (res.size() != 1) throw new DatabaseException("ERROR while inserting into db");
		...
		if (protocol.getCores() != null) weaver.setCores(protocol.getCores() + "");
		if (protocol.getMem() != null) weaver.setMemoryReq(protocol.getMem() + "");
		...
		if (isToWriteLocally) weaver.writeToFile(localLocation + pipelineElementNumber + scriptID, new String(
				pipelineScript.getScriptData()));
		...
		if (logpathfiles.size() > 0) for (int iii = 0; iii < logpathfiles.size(); iii++)
	...
	private Script makeJDLScript(String scriptID, String scriptRemoteLocation, String result)
	{
		...
		if (isToWriteLocally) weaver.writeToFile(localLocation + pipelineElementNumber + scriptID + ".jdl", new String(
				jdlfile.getBytes()));
