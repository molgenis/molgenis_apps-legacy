	...
	public void start()
	{
		...
		timer.scheduleAtFixedRate(new TimerTask()
		{
			public void run()
			{
				...
				// this is more efficient than read it every log file to find
				// application status later
				Vector<String> allLogs = readAllLogs();

				// System.out.println("size of active compute apps" +
				// activeComputeApps.size());
				...
				for (int i = 0; i < activeComputeApps.size(); i++)
				{
					...
					if (prevStatus.equals(status)) continue;
				...
				if (activeComputeApps.size() == 0) cancel();
	...
	protected ComputeAppPaths.AppStatus findStatus(String name, Vector<String> allLogs)
	{
		...
		for (int i = 0; i < allLogs.size(); i++)
		{
			...
			if (!logfile.equalsIgnoreCase(RemoteFileToStringReader.FILE_IS_NOT_EXISTS))
			{
				...
				if (index_finished > -1) return ComputeAppPaths.AppStatus.finished;
				else if (index_started > -1) return ComputeAppPaths.AppStatus.started;
			}
