...
python -c 'import os; a = lambda chromosome, ffrom, tto : [z[1] for z in [[y[1].split("_"), y[2]] for y in [x.split() for x in open("${impute2ResultsBinsLocation}")]] if z[0][0][3:] == chromosome and z[0][4] == str(ffrom) and z[0][5] == str(tto) and z[1] != "NO_SNP"]; b = lambda chromosome, ffrom, tto : ["getFile " + x for x in a(chromosome, ffrom, tto)] + [" ".join(["cat"] + a(chromosome, ffrom, tto) + [">", "_".join(["chr", chromosome, ffrom, tto])])]; [map(os.system, b("${chr}", str(x), str(x+${sampleBins}-1))) for x in range(0, ${samples}, ${sampleBins})]'
