...
public class WarningsAndErrors
{
	...
	public WarningsAndErrors(Data data, Database db, DataMatrixInstance modelInstance) throws DatabaseException
	{
		...
		List<ObservationElement> rowsInDb = db.find(ObservationElement.class, new QueryRule("name", Operator.IN,
				rowNamesOfInstance));
		List<ObservationElement> colsInDb = db.find(ObservationElement.class, new QueryRule("name", Operator.IN,
				colNamesOfInstance));
	...
	private List<String> checkForWarnings(Data data, Database db, DataMatrixInstance modelInstance,
			List<ObservationElement> rows, List<ObservationElement> cols) throws DatabaseException
	{
		List<String> warnings = new ArrayList<String>();

		if (data.getStorage().equals("BinaryFile"))
		{
			// BinaryMatrix instance = (BinaryMatrix)
			// model.getBrowser().getModel().getInstance();
			...
			{
				warnings.add("Column type conflict: Data declares " + data.getFeatureType()
						+ " while the original binary file was saved having " + instance.getData().getFeatureType());
			}
			if (!data.getTargetType().equals(instance.getData().getTargetType()))
			{
				warnings.add("Row type conflict: Data declares " + data.getTargetType()
						+ " while the original binary file was saved having " + instance.getData().getTargetType());
			}
			if (!data.getValueType().equals(instance.getData().getValueType()))
			{
				warnings.add("Value type conflict: Data declares " + data.getValueType()
						+ " while the original binary file was saved having " + instance.getData().getValueType());
			}
			if (!data.getInvestigation_Name().equals(instance.getData().getInvestigation_Name()))
			{
				warnings.add("Investigation name conflict: Data declares " + data.getInvestigation_Name()
						+ " while the original binary file was saved having "
						+ instance.getData().getInvestigation_Name());
			}
			if (!data.getName().equals(instance.getData().getName()))
			{
				warnings.add("Name conflict: Data declares " + data.getName()
						+ " while the original binary file was saved having " + instance.getData().getName());
			}
			if (!data.getStorage().equals(instance.getData().getStorage()))
			{
				warnings.add("Source type conflict: Data declares " + data.getStorage()
						+ " while the original binary file was saved having " + instance.getData().getStorage());
			}
		}
		...
		for (ObservationElement iden : rows)
		{
			...
			{
				warnings.add("Row element conflict: Identifiable " + iden.getName() + " is of type " + iden.get__Type()
						+ " while Data declares " + data.getTargetType());
			}
		}
		for (ObservationElement iden : cols)
		{
			...
			{
				warnings.add("Column element conflict: Identifiable " + iden.getName() + " is of type "
						+ iden.get__Type() + " while Data declares " + data.getFeatureType());
			}
		}
		return warnings;
	}
