...
public class RequestHandler
{
	...
	public static void handle(MatrixManagerModel screenModel, Tuple request, Database db) throws Exception
	{
		...
		else if (action.equals("changeSubmatrixSize"))
		{
			...
			// if only width has changed, do special update of submatrix to
			// preserve rows
			if (width != screenModel.getBrowser().getModel().getWidth()
					&& height == screenModel.getBrowser().getModel().getHeight())
			{
			...
			// if only height has changed, do special update of submatrix to
			// preserve columns
			else if (width == screenModel.getBrowser().getModel().getWidth()
					&& height != screenModel.getBrowser().getModel().getHeight())
			{
				setStepWidthHeight(screenModel, stepSize, width, height);
			...
			// both have changed, do a regular update of submatrix, resetting
			// row/col to limit/offset defaults
			else
			{
				setStepWidthHeight(screenModel, stepSize, width, height);
		...
		else if (action.startsWith("r_plot"))
		{
			...
			File img = MakeRPlot.plot(screenModel.getSelectedData(), instance, rowName, colName, action, type, width,
					height);
