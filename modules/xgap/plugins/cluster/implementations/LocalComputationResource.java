...
public class LocalComputationResource implements ComputationResource
{
	...
	public boolean cleanupJob(int jobId) throws Exception
	{
		...
		{
			commands.add(new Command("rm -rf " + tmpDir.getAbsolutePath() + "/runmij" + jobId + ".*", true, false,
					false));
			commands.add(new Command("rm -rf " + tmpDir.getAbsolutePath() + "/run" + jobId, true, false, false));
		}
		else
		{
			commands.add(new Command("del /f /s /q " + tmpDir.getAbsolutePath() + "\\runmij" + jobId + ".*", true,
					false, false));
	...
	/*
	 * private void installBiocPackage(String pkg, File usrHomeLibs, String OS)
	 * throws IOException{ File tmpDir = new
	 * File(System.getProperty("java.io.tmpdir")); File tmpFile = new
	 * File(tmpDir.getAbsoluteFile() + File.separator + "tmpRfile_" +
	 * System.nanoTime() + ".R"); String cmd;
	 * 
	 * if(OS.startsWith("windows")){ cmd =
	 * "echo source(\"http://bioconductor.org/biocLite.R\");biocLite(\""
	 * +pkg+"\",lib=\"" + usrHomeLibs.getAbsolutePath().replace("\\", "/") +
	 * "\");q(\"no\") > " + tmpFile.getAbsolutePath(); }else{ cmd =
	 * "echo source\\(\\\"http://bioconductor.org/biocLite.R\\\"\\)\\;biocLite\\(\\\""
	 * +pkg+"\\\",lib=\\\"" + usrHomeLibs.getAbsolutePath().replace("\\", "/") +
	 * "\\\"\\)\\;q\\(\\\"no\\\"\\) > " + tmpFile.getAbsolutePath(); }
	 * executeOSDependantCommand(new Command(cmd, true, false, false), OS);
	 * executeOSDependantCommand(new Command("R CMD BATCH " +
	 * tmpFile.getAbsolutePath(), true, false, false), OS); tmpFile.delete(); }
	 */
	...
	private void installRPackage(String pkg, String repos, File usrHomeLibs, String OS) throws Exception
	{
		...
		if (OS.startsWith("windows"))
		{
			cmd = "echo install.packages(\"" + pkg + "\",repos=\"" + repos + "\",lib=\""
					+ usrHomeLibs.getAbsolutePath().replace("\\", "/") + "\");q(\"no\") > " + tmpFile.getAbsolutePath();
		}
		else
		{
			cmd = "echo install.packages\\(\\\"" + pkg + "\\\",repos=\\\"" + repos + "\\\",lib=\\\""
					+ usrHomeLibs.getAbsolutePath().replace("\\", "/") + "\\\"\\)\\;q\\(\\\"no\\\"\\) > "
	...
	public boolean installDependencies() throws Exception
	{
		...
		// Again we check for missing libraries, and install then using tar.gz
		// packages
		if (OS.startsWith("unix") || OS.equals("mac"))
		{
			if (!bitopsDir.exists())
			{
				executeOSDependantCommand(
						new Command("R CMD INSTALL " + xgapRsources.getAbsolutePath() + File.separator
								+ "bitops_1.0-4.1.tar.gz --library=" + msWindowsSafePath(usrHomeLibs.getAbsolutePath())
								+ " --vanilla", true, false, false), OS);
			}
			...
			if (!qtlDir.exists())
			{
				...
				executeOSDependantCommand(
						new Command("R CMD INSTALL " + xgapRsources.getAbsolutePath() + File.separator
								+ "qtl_custom.tar.gz --library=" + msWindowsSafePath(usrHomeLibs.getAbsolutePath())
								+ " --vanilla", true, false, false), OS);
			...
			if (!rcurlDir.exists())
			{
				...
				executeOSDependantCommand(
						new Command("R CMD INSTALL " + xgapRsources.getAbsolutePath() + File.separator
								+ "RCurl_0.91-0.tar.gz --library=" + msWindowsSafePath(usrHomeLibs.getAbsolutePath())
								+ " --vanilla", true, false, false), OS);
