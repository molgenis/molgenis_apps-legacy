...
public class XqtlSeleniumTest
{
	...
	public static void deleteDatabase() throws Exception
	{
		...
		if (new UsedMolgenisOptions().mapper_implementation == MapperImplementation.JPA) // any
																							// JPA
																							// database
		{
			/**
		...
		// assuming HSQL standalone database, the xQTL default
		{
			...
			{
				// throw new
				// Exception("HSQL database directory does not exist");
			}

			// if (dbDir.list().length != 1)
			// {
			// throw new
			// Exception("HSQL database directory does not contain 1 file (.svn) after deletion! it contains: "
			// + dbDir.list().toString());
			// }
		}
	}
	...
	{
		selenium.waitForCondition("var value = selenium.isElementPresent('" + locator.replace("'", "\\'")
				+ "'); value == true", pageLoadTimeout);
	}
	...
	public void waitForElementToBeVisible(String locator)
	{
		...
		selenium.waitForCondition("var value = selenium.isVisible('" + locator.replace("'", "\\'")
				+ "'); value == true", pageLoadTimeout);
	}
	...
	public void userRoleMenuVisibility() throws Exception
	{
		...
		if (gbicdev_dontrunthis)
		{
			...
			Assert.assertTrue(selenium
					.isTextPresent("Home*Browse data*Upload data*Run QTL mapping*Configure analysis*Search / report*Utilities*Admin panel"));
			clickAndWait("id=Admin_tab_button");
			Assert.assertTrue(selenium
					.isTextPresent("Users and permissions*Database status*File storage*Install R packages*Admin utilities"));
	...
	public void namePolicy() throws Exception
	{
		...
		selenium.selectWindow("title=xQTL workbench");
		Assert.assertTrue(selenium
				.isTextPresent("ADD FAILED: Illegal character (#) in name '#'. Use only a-z, A-Z, 0-9, and underscore."));
		...
		Assert.assertTrue(selenium
				.isTextPresent("There is already a storage file named 'metaboliteexpression' which is used when escaping the name 'metaboliteExpression'. Please rename your Data matrix or contact your admin."));
	...
	public void databaseStatusPlugin() throws Exception
	{
		...
		Assert.assertTrue(selenium
				.isTextPresent("Investigation(s) present in the database, will not continue to load datamodel / reset db."));
	...
	public void fileStoragePlugin() throws Exception
	{
		...
		Assert.assertTrue(selenium
				.isTextPresent("Could not set file storage: Properties already present. Please delete first."));
	}
	...
	public void fileUploadPlugin() throws Exception
	{
		...
		// http://blog.browsermob.com/2011/03/selenium-tips-wait-with-waitforcondition/
		// "Now for the killer part, for sites that use jQuery, if all you need is to confirm there aren't any active asynchronous requests, then the following does the trick:"
		// selenium.waitForCondition("selenium.browserbot.getUserWindow().$.active == 0",
		// "10000");
		Thread.sleep(500);
	...
	{
		// check if the XREF options for Dataset / DataName / DataValue are
		// correct
		// behaviour: currently not well defined: but DataValue shows XREF
		// options outside
