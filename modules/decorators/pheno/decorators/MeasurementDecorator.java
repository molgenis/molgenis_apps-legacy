...
public class MeasurementDecorator<E extends Measurement> extends MapperDecorator<E>
{
	...
	private boolean makeCorrespondingProtocols(List<E> entities) throws DatabaseException
	{
		...
		{
			// In Hudson, there is no pre-generated Investigation 'System'
			// present in the DB,
		...
		for (E e : entities)
		{
			...
			try
			{
				...
				features_id.add(e.getId()); // e is already in the db so we can
											// do this
	...
	private boolean removeCorrespondingProtocols(List<E> entities) throws DatabaseException
	{
		...
		// To prevent Protocols from being removed when calling from Hudson
		// test:
		if (db.query(Investigation.class).eq(Investigation.NAME, "System").count() == 0)
		{
			// In Hudson, there is no pre-generated Investigation 'System'
	...
	{
		Database db = this.getDatabase();
		// To prevent Protocols from being updated when calling from Hudson
		// test:
		if (db.query(Investigation.class).eq(Investigation.NAME, "System").count() == 0)
		{
			// In Hudson, there is no pre-generated Investigation 'System'
		...
		/*
		 * String eventTypeName; String featureName;
		 * 
		 * for (Measurement e : entities) { featureName = e.getName(); // TODO:
		 * Do something here }
		 */
		return true;
	}
	...
	{
		// add your pre-processing here TODO: shouldn't we do this after the
		// update?
