...
/**
 * Utility class that makes it possible to Export Matrix data The idea is that
 * the Matrix interface is used!
 */
public class MatrixExporters
{
	...
	public static void getAsExcel(Database db, final SliceableMatrix matrix, OutputStream os) throws Exception
	{
		...
		for (int i = 0; i < colCount; i++)
		{
			for (int j = 0; j < rowCount; j++)
			{
				...
				{
					Label l = new Label(i, j + 1, elements[j][i].getValue().toString(), cellFormat);
					s.addCell(l);
				}
				else
				{
	...
	{
		/* Create tmp file */
		File excelFile = new File(System.getProperty("java.io.tmpdir") + File.separator + fileName + ".xls");
	...
	public static void getAsCSV(Database db, SliceableMatrix matrix, OutputStream os)
	{
