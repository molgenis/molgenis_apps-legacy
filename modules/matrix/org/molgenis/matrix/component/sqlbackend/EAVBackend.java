...
/**
 * Backend on EAV model, no tricks involved
 * 
 * @author joris lops
 */
public class EAVBackend
{
	...
	public EAVBackend(EntityManager em)
	{
		...
		MatrixQueryRule gewichtRule = new MatrixQueryRule(MatrixQueryRule.Type.colValueProperty, protocol.getId(),
				gewicht.getId(), Operator.EQUALS, "100.0");
		gewichtRule.setDimIndex(gewicht.getId());
		MatrixQueryRule lengteRule = new MatrixQueryRule(MatrixQueryRule.Type.colValueProperty, protocol.getId(),
				lengte.getId(), Operator.EQUALS, "141.8");
	...
	// ) AND
	// feature IN (51, 53)
	public String limitOffset(List<Measurement> feature, Measurement sortColumn, List<MatrixQueryRule> whereRules,
			int limit)
	{
		...
		String featureIds = ""; // join(extract(feature,
								// on(Measurement.class).getId()));
		// Lambdaj import doesn't work and "on" method cannot be found; Joris
		// please fix!
	...
	public String createWhereForProtocol(List<MatrixQueryRule> rules)
	{
		...
		for (MatrixQueryRule rule : rules)
		{
			if (rule.getFilterType().equals(MatrixQueryRule.Type.colValueProperty))
			{
				...
				wherePart.append(String.format("WHERE feature = %d AND CAST(value as NUMBER) > %s", m.getId(), value));
