 ...
 * model. The model works by constructing select queries based on group by and
 * max to recreate relational table look a like
 * 
 * @author jorislops
 */
public class EAVRelationalBackend implements Backend
{
	...
	public String createQuery(boolean count, List<MatrixQueryRule> rules)
	{
		...
		for (Map.Entry<Protocol, List<Measurement>> entry : measurementsByProtocol.entrySet())
		{
			...
			for (Measurement m : measurements)
			{
				...
				if (!count)
				{
					...
					// fields.append(String.format("%s.%s %s_%s",
					// entry.getKey().getName(), m.getName(),
					// entry.getKey().getName(), m.getName()));
					// } else {
			...
			// boolean paIDExists = exists(entry.getValue(),
			// having(on(Measurement.class).getName(),
			// IsEqualIgnoringCase.equalToIgnoringCase(JOIN_COLUMN)));
			...
			if (!paIDExists)
			{
				Measurement paIdMes = em
						.createQuery(
								"SELECT DISTINCT m FROM Measurement m JOIN m.featuresCollection p WHERE m.name = :name and p = :protocol",
								Measurement.class).setParameter("name", joinColumn)
						.setParameter("protocol", entry.getKey()).getSingleResult();
			...
			else
			{
				...
				query.append(String.format(" left join (%s) %s on (%s = %s)", sql, protocolName, prevJoinColumn,
						aliasName));
			}
		...
		if (!StringUtils.isEmpty(whereFilter))
		{
			...
			{
				query = new StringBuilder(String.format("SELECT %s FROM (%s) WHERE %s", fields, query.toString(),
						whereFilter));
			}
		}
		...
		if (!count)
		{
			...
			if (sortProtocol != null)
			{
				...
				query.append(String.format(" ORDER BY %s_%s %S", sortProtocol.getName(), measurementProtocol.getName(),
						sortOrder));
			}
		}
		...
		if (count)
		{
			...
			// todo: optimalisation, remove columns that are not needed for
			// count!
			// kkfw
			// // for(List<Measurement> m : pM.values()) {
			// // List<Measurement> result =
			// filter(having(on(Measurement.class).getName(),
			// IsIn.isIn(asList("PA_ID"))), m);
			// //
			// // }
		}
		else
