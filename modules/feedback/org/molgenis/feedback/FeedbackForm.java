...
public class FeedbackForm extends PluginModel<Entity>
{
		...
		try
		{
			...
			if ("sendFeedback".equals(request.getAction()))
			{
				...
				if (StringUtils.isEmpty(admin.getEmail())) throw new DatabaseException(
						"Sending feedback failed: the administrator did not enter an email address. Please contact your administrator about this.");
	...
	public String getActivePlugin()
	{
		...
		{
			return "Home"; // need a nice default when no screen is explicitly
							// selected... should be the screen that is
							// currently shown, though! might be easy to fix?
		}
